...
from infrastructure.persistence.layout.data_home import ensure_elfie_home, get_env_path

_ENV_NAME_PATTERN = re.compile(r"[^A-Z0-9_]+")
_CONNECTION_ID_PATTERN = re.compile(r"^[a-z][a-z0-9_]{0,63}$")


def provider_secret_name(
    provider_id: str,
    *,
    configured_name: str | None = None,
) -> str:
    """Return the stable environment name for a provider credential.

    Provider catalog metadata belongs to the model/configuration boundary, not
    persistence.  Callers that already resolved a catalog-specific name may
    pass it explicitly; persistence keeps only the deterministic fallback.
    """
    if configured_name:
...
def set_provider_secret(
    ...
    *,
    ...
    name = provider_secret_name(provider_id, configured_name=configured_name)
