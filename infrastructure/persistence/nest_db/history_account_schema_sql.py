...
ACCOUNT_CONVERSATION_STATEMENTS: Final[tuple[str, ...]] = (
    ...
    WHEN
        (NEW.self_account_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM conversations AS c
            JOIN self_channel_accounts AS a ON a.self_account_id = NEW.self_account_id
            WHERE c.conversation_id = NEW.conversation_id AND a.channel <> c.channel
        ))
        OR (NEW.channel_account_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM conversations AS c
    ...
    BEFORE INSERT ON conversation_participants
    WHEN NEW.participant_type = 'self' AND NOT EXISTS (
        SELECT 1 FROM conversations AS c
