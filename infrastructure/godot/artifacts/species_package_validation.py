...
def validate_source_species_packages(
    *,
    ...
    expected_packages = tuple(
        definition.godot_package_id for definition in definitions
    )
    ...
    for definition in definitions:
        ...
        if not isinstance(manifest, dict) or manifest.get("species_id") != definition.species_id:
            raise SpeciesPackageValidationError(
        ...
        for key in ("scene_file", "model_file"):
            ...
            if not isinstance(asset_name, str) or not (package_root / asset_name).is_file():
                raise SpeciesPackageValidationError(
    ...
    try:
        ...
        raise SpeciesPackageValidationError("godot-species-validation-failed") from error
    ...
    try:
        ...
        raise SpeciesPackageValidationError("godot-species-validation-marker-invalid") from error
