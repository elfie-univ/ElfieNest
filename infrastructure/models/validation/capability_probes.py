...
_VISION_PIXEL = base64.b64encode(
    bytes.fromhex(
        "89504e470d0a1a0a0000000d49484452000000010000000108060000001f15c4890000000d49444154789c6360f8cf00000004000101a2a0ed0000000049454e44ae426082"
    )
).decode("ascii")
    ...
    *,
    ...
    for observation in sorted(
        ...
        if observation.details.get("evidence_kind") != "capability":
            continue
        if config_fingerprint is not None and observation.details.get(
            "config_fingerprint"
        ...
        capability_name = cast(CapabilityName, raw_name)
        capability_state = cast(CapabilityState, raw_state)
        capability_evidence = cast(CapabilityEvidence, raw_evidence)
        if projected[capability_name].evidence in {"verified", "accepted"}:
            continue
        projected[capability_name] = EndpointCapability(
            capability_name,
            capability_state,
            capability_evidence,
        )
    ...
    return tuple(projected[name] for name in capability_names)


def _vision_content(api_mode: str) -> Any:
    prompt = (
        "Inspect the image and return exactly ELFIENEST_VISION_OK if you can read it."
    )
    ...
    if api_mode == "codex_responses":
        return [
            ...
            {
                "type": "input_image",
                "image_url": f"data:image/png;base64,{_VISION_PIXEL}",
            },
        ]
    return [
        {"type": "text", "text": prompt},
...
def _verified(
    capability: CapabilityName, reason: str, **details: Any
) -> CapabilityProbeResult:
    return CapabilityProbeResult(
        capability,
        "supported",
