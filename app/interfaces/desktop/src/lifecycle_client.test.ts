...
function commandRunner(
  ...
  return {
    calls,
    run: async (command: string, argumentsList: readonly string[]): Promise<string> => {
...
test("managed lifecycle client attaches to a ready CLI-owned Runtime without starting it", async () => {
  ...
  const client = new ManagedRuntimeLifecycleClient("elfienest", "desktop-11", runner);
  ...
  assert.deepEqual(runner.calls, [{ command: "elfienest", argumentsList: ["status", "--json"] }]);
});
...
test("managed lifecycle client starts a stopped Runtime with its desktop lease", async () => {
  ...
  const client = new ManagedRuntimeLifecycleClient("elfienest", "desktop-12", runner);
  ...
  assert.deepEqual(runner.calls, [
    { command: "elfienest", argumentsList: ["status", "--json"] },
    { command: "elfienest", argumentsList: ["start", "--owner-id", "desktop-12"] },
    { command: "elfienest", argumentsList: ["status", "--json"] },
  ]);
});
...
test("managed lifecycle client sends a lease-scoped stop request", async () => {
  ...
  const client = new ManagedRuntimeLifecycleClient("elfienest", "desktop-13", runner);
  ...
  assert.deepEqual(runner.calls, [
    { command: "elfienest", argumentsList: ["stop", "--owner-id", "desktop-13"] },
  ]);
});
