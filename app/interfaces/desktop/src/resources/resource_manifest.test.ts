...
test("resource manifest records and validates every packaged component for one staging target", () => {
  ...
  try {
    ...
    mkdirSync(join(root, "config/genesis/species/saevi/assets"), { recursive: true });
    writeFileSync(join(root, "config/genesis/species/catalog.yaml"), "version: 1\n");
    writeFileSync(join(root, "config/genesis/species/saevi/assets/headshot.png"), "png");
    ...
    assert.ok(manifest.files["config/genesis/species/catalog.yaml"]);
    assert.ok(manifest.files["config/genesis/species/saevi/assets/headshot.png"]);
