...
export function showStartupFailure(window: BrowserWindow, error: unknown): void {
  ...
  const document = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>ElfieNest 启动失败</title><style>body{font:16px system-ui,sans-serif;background:#101418;color:#e8edf2;padding:48px}main{max-width:600px;margin:auto}h1{font-size:26px}pre{white-space:pre-wrap;background:#1b222a;border:1px solid #33404c;border-radius:8px;padding:16px;color:#ffb4ab}.actions{display:flex;gap:12px;margin-top:20px}a,button{padding:10px 18px;border:0;border-radius:6px;background:#8bd5ca;color:#10201e;font-weight:600;cursor:pointer;text-decoration:none}a.secondary{background:#29333d;color:#e8edf2}</style><main><h1>ElfieNest 无法启动</h1><p>请根据下面的组件错误检查安装资源或本机权限。</p><pre>${safeMessage}</pre><div class="actions"><a href="elfienest://retry-runtime">重试</a><button onclick="window.close()">关闭窗口</button></div></main></html>`;
  loadDocument(window, document);
}
