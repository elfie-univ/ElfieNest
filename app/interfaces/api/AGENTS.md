...
WebSocket 规则，并定义产品 API 的长期契约和迁移期边界。当前用户指令和更高层
...
- 产品 API 默认位于 `/api/v1`。除本文件明确列出的基础设施例外外，不得新增未经
  用户批准的非 `/api/v1` 产品接口。
- 调用方迁移完成后立即删除被替代的旧 Route、Client、DTO 和测试夹具；MVP 阶段
  不保留长期兼容壳、双路由、fallback read 或字段 alias。
...
本节只在旧接口尚未完成迁移期间生效。所有产品 Route 和调用方符合以上长期契约、
旧路径清零后，应删除本节并保留核心原则中的长期替换规则。

迁移按用户批准的业务域逐个闭环，不一次性实现整站新 Route，也不长期并行维护两套
完整接口。每个业务域按以下顺序执行：

1. 盘点旧 Route、生产与测试调用方、Feature/Repository、目标路径和删除条件；
2. 先搜索并确认可复用能力，为目标契约增加直接相关测试；
3. 建立该业务域的目标 Route、严格请求/响应模型和统一错误映射；
4. 迁移 Web、Setup、CLI、Observer、设备或脚本中的全部真实调用方；
5. 验证新调用链后，立即删除该域旧 Route、Client、DTO、测试夹具和兼容分支；
6. 搜索旧路径、旧字段、松散模型和只被测试保护的响应，确认该域没有残留。

删除前必须区分页面漏接、外部协议、已确认未来能力和明确被新接口替代的历史实现。
不得以“路由已注册”证明接口有价值，也不得以“前端没调用”直接证明接口无用。

迁移涉及的 architecture 门禁应随着已迁移范围逐步收紧，至少防止：新增未版本化
产品 Route、恢复已删除路径、API 层直接 SQL、新增松散请求/响应、重复注册同一
method + path，以及前端页面硬编码 API。不得维护永久 allowlist 掩盖旧接口。
