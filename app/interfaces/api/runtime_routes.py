...
def build_runtime_status() -> Dict[str, Any]:
    ...
    fallback_connection = next(
        (
            connection
            for connection in connections.values()
            if connection.catalog_id == "ollama" and connection.enabled
        ),
        None,
    )
    fallback_configured = fallback_connection is not None
    ...
    return {
        ...
        "fallback": {
            ...
            "api_base": fallback_connection.api_base if fallback_connection else "",
        },
        "tools": {
...
def _merge_runtime_policy(
    ...
    if "task_routes" in updates:
        raise HTTPException(
            status_code=410,
            detail="task_routes 已停用；大脑只请求语义角色",
        )
    for field_name in ("tool_permissions",):
