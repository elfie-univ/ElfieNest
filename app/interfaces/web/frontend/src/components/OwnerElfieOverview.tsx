...
export function OwnerElfieOverview({ onCountChange }: OwnerElfieOverviewProps) {
  ...
  return <section className="manage-card manage-card--wide">
    ...
    <div className="manager-filters">
      <label>所属用户<select aria-label="按用户筛选精灵" onChange={(event) => update("ownerUserId", event.target.value)} value={filters.ownerUserId ?? ""}><option value="">全部用户</option>{users.map((user) => <option key={user.id} value={String(user.id)}>{user.username}</option>)}</select></label>
