...
export function AccountMenuPanel({ onClose, onUpdated, user }: AccountMenuPanelProps) {
  ...
  return <section aria-label="个人与外观设置" className="account-menu__panel">
    ...
    {user.role === "owner" ? <SettingRow active={expanded === "landing"} icon="house" label="默认登录页" onToggle={() => toggle("landing")} summary={sectionSummary("landing", user)}>
      <div className="account-menu__landing"><SelectField label="默认登录页" onValueChange={(value) => setLanding(value === "chat" ? "chat" : "manage")} options={[{ label: "管理页", value: "manage" }, { label: "聊天页", value: "chat" }]} value={landing} /><Button className="account-menu__landing-action" disabled={saving === "landing"} onClick={() => { void saveLanding() }} type="button">{saving === "landing" ? "正在保存…" : "保存默认页"}</Button></div>
    </SettingRow> : null}
