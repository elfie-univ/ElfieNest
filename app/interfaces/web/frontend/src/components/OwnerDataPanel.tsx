...
export function OwnerDataPanel({ title, description, readPath, csrfToken, writePath }: OwnerDataPanelProps) {
  ...
  return <section className="manage-card"><div className="manage-head"><div><h2>{title}</h2><p>{description}</p></div><button className="button button--quiet" onClick={() => { void load() }} type="button">刷新</button></div>{error && <Notice kind="error" message={error} />}{notice && <Notice message={notice} />}{writePath === undefined ? <pre className="manager-json">{data === null ? "正在加载…" : renderJson(data)}</pre> : <form onSubmit={(event) => { void save(event) }}><textarea aria-label={`${title} JSON 配置`} className="manager-json-input" onChange={(event) => setEditor(event.target.value)} value={editor} /><button className="button" type="submit">保存配置</button></form>}</section>
}
