  ...
  return <section className="nest-side-card bed-distribution"><h3>床位分布</h3>
    <ul aria-label="床位分布">{ordered.map((elfie) => {
      ...
      return <li key={elfie.elfie_id}>
        <div className="bed-distribution__summary"><Avatar imageUrl={elfie.profile.portrait_url} name={elfie.profile.name} /><span><strong>{elfie.profile.name}</strong><small>{bed?.name ?? "未分配"}</small></span><button aria-label={`编辑${elfie.profile.name}的床位`} className="icon-button" onClick={() => beginEdit(elfie)} type="button"><Icon name="pencil" size={16} /></button></div>
        {editing ? <div className="bed-distribution__editor"><SelectField ariaLabel={`为${elfie.profile.name}选择床位`} onValueChange={setSelection} options={[{ label: "未分配", value: "unassigned" }, ...beds.map((item) => ({ disabled: Boolean(item.occupant_id && item.occupant_id !== elfie.elfie_id), label: `${item.name}${item.occupant_name ? ` · ${item.occupant_name}` : " · 空闲"}`, value: item.anchor_id }))]} value={selection} /><div className="manage-actions"><button className="button" disabled={saving} onClick={() => { void save(elfie.elfie_id) }} type="button">{saving ? "保存中…" : "保存"}</button><button className="button button--quiet" disabled={saving} onClick={() => setEditingId(null)} type="button">取消</button></div></div> : null}
      </li>
    })}</ul>
  </section>
}
