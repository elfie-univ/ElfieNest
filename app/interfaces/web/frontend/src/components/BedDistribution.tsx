...
import type { AdminElfie, NestBed, NestRoom } from "../api/client"
...
type BedDistributionProps = {
  readonly elfies: readonly AdminElfie[]
  readonly onAssign: (elfieId: string, anchorId: string | null) => Promise<boolean>
  readonly rooms: readonly NestRoom[]
}

function assignedBed(elfie: AdminElfie, beds: readonly NestBed[]): NestBed | undefined {
  return beds.find((bed) => bed.occupant_id === elfie.profile.elfie_id)
}
...
export function BedDistribution({ elfies, onAssign, rooms }: BedDistributionProps) {
  ...
  const beginEdit = (elfie: AdminElfie): void => {
    setEditingId(elfie.profile.elfie_id)
  ...
  return <section className="nest-side-card bed-distribution"><h3>{t("nest.assignment.title")}</h3>
    <ul aria-label={t("nest.assignment.listLabel")}>{ordered.length === 0 ? <li className="bed-distribution__empty">{t("nest.assignment.empty")}</li> : ordered.map((elfie) => {
      ...
      const editing = editingId === elfie.profile.elfie_id
      return <li key={elfie.profile.elfie_id}>
        ...
        {editing ? <div className="bed-distribution__editor"><SelectField label={t("nest.assignment.bedFor", { name: elfie.profile.name })} onValueChange={setSelection} options={[{ label: t("nest.assignment.unassigned"), value: "unassigned" }, ...beds.map((item) => ({ disabled: Boolean(item.occupant_id && item.occupant_id !== elfie.profile.elfie_id), label: item.occupant_name ? `${item.name}${t("nest.assignment.occupiedSuffix", { name: item.occupant_name })}` : `${item.name}${t("nest.assignment.freeSuffix")}`, value: item.anchor_id }))]} value={selection} /><div className="manage-actions"><Button disabled={saving} onClick={() => { void save(elfie.profile.elfie_id) }} type="button">{saving ? t("nest.actions.saving") : t("nest.actions.save")}</Button><Button variant="outline" disabled={saving} onClick={() => setEditingId(null)} type="button" >{t("nest.actions.cancel")}</Button></div></div> : null}
      </li>
    })}</ul>
  </section>
}
