...
export function OwnerProviderPanel({ csrfToken }: { readonly csrfToken: string }) {
  ...
  return <section className="manage-card manage-card--wide">
    ...
    <form className="provider-form" onSubmit={(event) => { void save(event) }}><h3>{editing ? `编辑 ${draft.providerId}` : "添加供应商"}</h3><input disabled={editing} onChange={(event) => change("providerId", event.target.value)} placeholder="供应商 ID" required value={draft.providerId} /><input onChange={(event) => change("displayName", event.target.value)} placeholder="显示名称" value={draft.displayName} /><input onChange={(event) => change("apiBase", event.target.value)} placeholder="API Base URL" type="url" value={draft.apiBase} /><input autoComplete="new-password" onChange={(event) => change("apiKey", event.target.value)} placeholder={editing ? "留空则不变更密钥" : "API 密钥（如需要）"} type="password" value={draft.apiKey} /><SelectField ariaLabel="供应商接口协议" onValueChange={(value) => change("apiMode", value)} options={[{ label: "Chat Completions", value: "chat_completions" }, { label: "Anthropic Messages", value: "anthropic_messages" }, { label: "Ollama", value: "ollama" }]} value={draft.apiMode} /><SelectField ariaLabel="供应商认证方式" onValueChange={(value) => change("authType", value)} options={[{ label: "Bearer", value: "bearer" }, { label: "X-API-Key", value: "x-api-key" }, { label: "无认证", value: "none" }]} value={draft.authType} /><input onChange={(event) => change("testModel", event.target.value)} placeholder="连通性测试模型（可选）" value={draft.testModel} /><input onChange={(event) => change("models", event.target.value)} placeholder="模型 ID，以 | 分隔" value={draft.models} /><div className="manage-actions"><button className="button" type="submit">保存供应商</button>{editing && <button className="button button--quiet" onClick={() => { setDraft(EMPTY_DRAFT); setEditing(false) }} type="button">取消编辑</button>}</div></form>
  </section>
}
