...
export function OwnerModelPanel({ csrfToken }: { readonly csrfToken: string }) {
  ...
  return <section className="manage-card manage-card--wide"><div className="manage-head"><div><h2>模型</h2><p>管理目录可见性与成本层级；可扫描本地 Ollama 模型。</p></div><div className="manage-actions"><RefreshButton label="刷新" onClick={() => { void load() }} /><Button variant="outline" onClick={() => { void scan() }} type="button">扫描本地模型</Button></div></div>{error && <Notice kind="error" message={error} />}{notice && <Notice message={notice} />}<div className="catalog-table">{models.map((model) => <article key={model.model_id}><strong>{model.display_name}</strong><small>{model.provider} · {model.active ? "可用" : "未激活"} · {model.capabilities.join("、")}</small><label><Checkbox checked={model.visible} onCheckedChange={(checked) => { void update(model, { visible: checked === true }) }} /> 在管理目录显示</label><SelectField label={`${model.display_name} 成本层级`} onValueChange={(value) => { void update(model, { cost_tier: Number(value) }) }} options={[0, 1, 2, 3, 4].map((tier) => ({ label: String(tier), value: String(tier) }))} value={String(model.cost_tier)} /></article>)}</div></section>
}
...
export function OwnerToolPanel({ csrfToken }: { readonly csrfToken: string }) {
  ...
  return <section className="manage-card manage-card--wide"><div className="manage-head"><div><h2>工具</h2><p>所有工具设置和验证均经 Owner API；密钥字段只写不读。</p></div><RefreshButton label="刷新" onClick={() => { void load() }} /></div>{error && <Notice kind="error" message={error} />}{notice && <Notice message={notice} />}<div className="tool-editor"><div className="tool-list">{Object.entries(tools).map(([key, value]) => <button className={selectedKey === key ? "list-row list-row--active" : "list-row"} data-slot="button" data-variant="ghost" key={key} onClick={() => select(key)} type="button"><strong>{key}</strong><small>{JSON.stringify(value)}</small></button>)}</div><div><FieldRow control={({ describedBy, inputId, labelId }) => <Textarea aria-describedby={describedBy} aria-labelledby={labelId} className="manage-json-input" disabled={!selectedKey} id={inputId} onChange={(event) => setEditor(event.target.value)} value={editor} />} inputId="tool-json-config" label="工具 JSON 配置" /><div className="manage-actions"><Button disabled={!selectedKey} onClick={() => { void save() }} type="button">保存工具</Button><Button variant="outline" disabled={!selectedKey} onClick={() => { void verify() }} type="button">验证工具</Button></div></div></div></section>
}
