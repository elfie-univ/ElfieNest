...
describe("OwnerFoodPanel", () => {
  ...
  it("edits only the selected recipe through role controls", async () => {
    const user = userEvent.setup()
    render(<OwnerFoodPanel csrfToken="csrf" />)

    await user.click(await screen.findByRole("button", { name: "编辑 标准粮" }))
    const dialog = screen.getByRole("dialog", { name: "编辑 标准粮" })
    expect(within(dialog).getByRole("combobox", { name: "主模型" })).toBeInTheDocument()
    expect(within(dialog).getByRole("combobox", { name: "深度模型" })).toBeInTheDocument()
    expect(within(dialog).getByRole("combobox", { name: "校验模型" })).toBeInTheDocument()
    expect(within(dialog).getByRole("button", { name: "添加技术回退" })).toBeInTheDocument()

    await user.click(within(dialog).getByRole("button", { name: "保存标准粮" }))
