...
export function ObserverSurface(props: ObserverSurfaceProps) {
  ...
  return <section aria-label={props.title} className="observer-surface">
    <div className="observer-surface__head"><div><strong>{props.title}</strong><small>{statusCopy}</small></div>{status === "idle" ? <button className="button" onClick={open} type="button"><Icon name="cuboid" size={16} />进入 3D</button> : <button className="button button--quiet" onClick={() => observer?.detach()} type="button">结束观察</button>}</div>
    <div className={status === "ready" || status === "loading" ? "observer-surface__viewport" : "observer-surface__fallback"} ref={surfaceRef}>
      ...
      {status === "fallback" ? <><p>当前设备无法运行 3D 观察，已保留语义状态和页面操作。</p><p>{entityCount > 0 ? `当前可见 ${entityCount} 位精灵。` : "可继续使用聊天、资料和房间管理。"}</p>{observer !== null ? <button className="button" onClick={open} type="button">重试 3D</button> : null}</> : null}
    </div>
  </section>
}
