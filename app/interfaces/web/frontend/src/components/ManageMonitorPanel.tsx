...
export function ManageMonitorPanel() {
  ...
  const load = useCallback(async (): Promise<void> => {
    setLoading(true)
    setSnapshot(await loadMonitorSnapshot())
    setLoading(false)
  }, [])
  ...
  return <section className="monitor-panel">
    ...
    {authRequired && <Notice kind="error" message={t("runtimeMonitor.authRequired")} />}
    {allSourcesFailed && <Notice kind="error" message={t("runtimeMonitor.allLoadFailed")} />}
    {partiallyLoaded && <p className="monitor-partial-notice">{t("runtimeMonitor.partialLoad")}</p>}
