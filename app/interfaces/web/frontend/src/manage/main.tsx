...
async function loadState(): Promise<ManageState> {
  ...
  return { user, users, elfies, rooms }
}
  ...
  return (
    <main className="page"><section className="panel manage">
      ...
      <section className="manage-grid">
        ...
        <ManageCard description="聊天页与管理页共享同一会话。供应商、模型、工具、粮食与系统设置暂由兼容管理工作区承载，二期再完成其页面迁移。" title="会话与入口"><div className="manage-actions"><a className="button" href="/chat">进入聊天</a><a className="button button--quiet" href="/manage?mode=classic">高级运行配置</a><button className="button button--quiet" onClick={() => void leave()} type="button">退出登录</button></div></ManageCard>
      </section>
