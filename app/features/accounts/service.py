...
class AccountsService:
    ...
    def update_profile(
        ...
        birth_date = (
            command.birth_date
            if "birth_date" in command.fields
    ...
    def list_managed_accounts(
        ...
        return ManagedAccountsResult(
            items=tuple(self._managed_result(record, default_limit) for record in records)
        )
    ...
    def _validate_identity(account_id: str, display_name: Optional[str]) -> None:
        ...
        if display_name is not None and len(display_name.strip()) > _MAX_DISPLAY_NAME_LENGTH:
