...
- 本目录遵守父级 `app/AGENTS.md`：持久化类是 Feature/Orchestration Port 的具体
  Adapter，不拥有产品授权、业务流程或协议 DTO。持久化 Record 只在 Adapter 内部
  使用，不能作为 API 响应或跨层通用模型。
...
- 查询 Adapter 只读取权威事实或明确的派生投影，不在读取过程中偷偷写入、修复或
  迁移数据。新建索引、缓存表或投影时必须声明权威源与重建方式。
...
- 新增永久数据路径必须通过统一数据根 resolver；当前
  `ai_runtime.storage.data_home` 是待迁移实现，不得成为新的目标所有权。resolver 的
  目标位置由后续持久化/平台迁移切片确定。只有永久数据契约或开发者可见行为变化时
  才同步中英文文档。
