...
MESSAGE_STATEMENTS: Final[tuple[str, ...]] = (
    ...
    CREATE TABLE IF NOT EXISTS messages (
        ...
        CHECK(
            (sender_type = 'self' AND direction = 'outbound'
                AND self_account_id IS NOT NULL AND channel_account_id IS NULL)
            OR (sender_type = 'external' AND direction = 'inbound'
                AND self_account_id IS NULL AND channel_account_id IS NOT NULL)
            OR (sender_type = 'internal' AND direction = 'internal'
                AND self_account_id IS NULL AND channel_account_id IS NULL)
        ),
        UNIQUE(channel, source_message_key),
        UNIQUE(message_id, conversation_id),
        FOREIGN KEY(conversation_id) REFERENCES conversations(conversation_id),
        FOREIGN KEY(conversation_id, channel)
            REFERENCES conversations(conversation_id, channel),
        ...
        FOREIGN KEY(channel_account_id)
            REFERENCES external_channel_accounts(channel_account_id),
    ...
    WHEN
        (NEW.sender_type = 'self' AND NOT EXISTS (
            SELECT 1 FROM conversations AS c
            WHERE c.conversation_id = NEW.conversation_id
                AND c.self_account_id = NEW.self_account_id
        ))
        OR (NEW.sender_type = 'external' AND NOT EXISTS (
            SELECT 1 FROM conversation_participants AS p
    ...
    ON messages(self_account_id, created_at) WHERE self_account_id IS NOT NULL
    """,
    """
