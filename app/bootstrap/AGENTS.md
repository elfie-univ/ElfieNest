...
- 正式安装启动只能装配包内已有可执行文件和静态资源；资源缺失时返回 preflight 错误，
  不在启动链路安装依赖、导出 Godot 或构建产品资源。
- 不写授权、业务条件、SQL、协议 DTO 映射、模型推荐算法或第二套配置事实。
- 只装配 Genesis 所需的强类型资料 Loader、最终 owner Store 与调用边界；不得在组合根
  实现个人知识、人物、关系、经历或人格生成规则。
- 配置只在这里装配：注入统一的内置资源根、用户配置 Adapter 或已经校验的强类型结果；
  不自行读取 YAML、执行合并、复制默认值或把源码 checkout 当安装态回退路径。
