...
class TinyVectorStorage:
    ...
    def __init__(self, storage_path: str = None):
        if storage_path is None:
            ...
            # 向上寻找到根目录
            self.storage_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(current_dir))), ".elfie_memories.json")
        else:
    ...
    def _load_from_disk(self):
        if os.path.exists(self.storage_path):
            try:
                ...
                logger.info(f"💾 [记忆读取完毕] 成功加载 {len(self.memories)} 条历史情景索引。")
    ...
    def retrieve_relevant_memories(self, query: str, top_k: int = 2) -> List[str]:
        ...
        for entry in self.memories:
            ...
            if not intersection:
                ...
                # 包含度计算
                score = len(intersection) / (math.sqrt(len(query_words)) * math.sqrt(len(content_words)))
