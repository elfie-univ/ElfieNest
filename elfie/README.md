...
可替换身体、通信和技能；能量、情绪、运行时间和当前身体绑定由对应模块在进程
内维护。房间、床位、多精灵调度、用户归属和产品
界面属于 `elfienest/`，模型供应商与底层工具执行属于 `runtime/`。

## 精灵个体刻画框架

一只完整精灵由稳定档案、长期记忆和各模块当前运行值共同刻画：
...
- 每个 tick 都在变化
- 重启后由模块默认值重新开始
- `elapsed_time` 与当前身体绑定只属于本次进程，不写入稳定档案
...
初始化精灵 ← Profile (定义"我是谁")
     ↓
运行精灵 ← Runtime (各模块维护"现在怎么样")
     ↓
成长精灵 ← Memory (沉淀"经历了什么、理解了什么")
```

- **Profile** 初始化精灵，定义稳定身份
- **Runtime** 由能量、情绪、身体绑定等模块维护当前进程内的瞬时值
- **Memory** 沉淀成长轨迹和世界认知

这些部分合起来，完整刻画了一个精灵：它是谁、它现在怎么样、它经历了什么、理解了什么。
...
`communication/` 和 `skills/`。旧 `interface/`、源码级 `config/`、
