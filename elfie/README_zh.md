...
- `elfie.brain.EventWorkspace`：把类型化事件接纳为相互隔离的 Turn；
- `elfie.brain.BrainCoordinator`：生成认知帧、上下文和思考运行；
- `elfie.brain.DecisionPlan`：思考中枢输出的类型化决定；
- `elfie.brain.reasoning.execution_router.OutputRouter`：把决策路由到身体、通信或 Activity。
