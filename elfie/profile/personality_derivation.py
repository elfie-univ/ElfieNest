...
def _derive_preset_scores(seed: int, preset: str) -> Mapping[str, float]:
    ...
    for trait, (lower, upper) in PERSONALITY_PRESETS[preset].items():
        material = f"{seed}:{preset}:{trait}".encode()
...
def _load_default_big_five() -> Mapping[str, float]:
    defaults_path = Path(__file__).parent / "defaults" / "personality.yaml"
    with defaults_path.open(encoding="utf-8") as defaults_file:
        document = yaml.safe_load(defaults_file)
    try:
        big_five = document["big_five"]
        return {trait: float(big_five[trait]) for trait in BIG_FIVE_TRAITS}
    except (KeyError, TypeError, ValueError) as error:
        raise PersonalityDerivationError(
            f"默认人格配置无效: {defaults_path}"
        ) from error
