...
class VoiceProfile:
    """精灵声音声学特性曲线 profile"""
    def __init__(self, pitch: float = 1.0, speed: float = 1.0, timbre: str = "cute", frequency_curve: List[float] = None):
        self.pitch = pitch              # 音高倍率 (0.5 - 2.0)
        self.speed = speed              # 语速倍率 (0.5 - 2.0)
        self.timbre = timbre            # 音色风格描述
        # 频率特性曲线，表示不同赫兹段的共振衰减值 (如 10个频段的 gain 数组)
        self.frequency_curve = frequency_curve or [1.0, 1.2, 1.5, 1.3, 1.0, 0.8, 0.7, 0.9, 1.1, 1.0]
...
class JointLimit:
    """数字孪生可动关节描述"""
    def __init__(self, name: str, min_angle: float, max_angle: float, current_angle: float = 0.0):
        self.name = name
...
class SomaticAnatomy:
    ...
    def add_joint(self, name: str, min_angle: float, max_angle: float, current_angle: float = 0.0):
