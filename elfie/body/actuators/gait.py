...
class GaitEngine:
    ...
    def generate_step_angles(self, anatomy: SomaticAnatomy, gait_type: str, speed: float, elapsed_time: float) -> Dict[str, float]:
        """
        ...
        if gait_type == "idle":
            ...
            if "tail_wag" in anatomy.joints:
                angles["tail_wag"] = 0.05 * math.sin(2.0 * math.pi * 0.1 * elapsed_time) # 尾巴微摇
            return angles
        ...
        if isinstance(anatomy, BipedAnatomy):
            if gait_type in ["walk", "run"]:
                ...
                angles["right_knee"] = abs(amplitude * 1.2 * math.sin(t + math.pi - math.pi/4))
