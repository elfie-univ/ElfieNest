...
def apply_transfer(emotions: Dict[str, float], source: str, target: str, config: dict) -> float:
    """
    应用转移交互。
...
def get_inhibition_modifier(emotions: Dict[str, float], source: str, rate: float, max_value: float = 100.0) -> float:
    """
    计算抑制系数。
...
def get_enhancement_modifier(emotions: Dict[str, float], source: str, rate: float, max_value: float = 100.0) -> float:
    """
    计算增强系数。
...
class EmotionInteractionSystem:
    ...
    def __init__(self, interactions: Optional[Dict[Tuple[str, str], Dict[str, Any]]] = None):
        """
        初始化情绪交互系统。
        ...
        """
        self.interactions = interactions if interactions is not None else EMOTION_INTERACTIONS
    
    def apply_transfer_interactions(self, emotions: Dict[str, float]) -> Dict[Tuple[str, str], float]:
        """
        应用所有转移交互（在tick中调用）。
    ...
    def get_accumulate_modifier(self, target_emotion: str, emotions: Dict[str, float], max_value: float = 100.0) -> float:
        """
        ...
        for (source, target), config in self.interactions.items():
            ...
            if config['type'] == 'inhibition':
                ...
                rate = config['rate']
                source_modifier = get_inhibition_modifier(emotions, source, rate, max_value)
                modifier *= source_modifier
            
            elif config['type'] == 'enhancement':
                # 增强：source存在时，target增长提高
                rate = config['rate']
                source_modifier = get_enhancement_modifier(emotions, source, rate, max_value)
                modifier *= source_modifier
