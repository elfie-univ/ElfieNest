...
from elfie.brain.brain_types import SensorData
...
class ExpectationManager:
    ...
    def update_and_calculate_error(self, sensors: SensorData) -> float:
        """
        根据现实世界的传感器反馈，对比脑内预期，算出预测误差
        :param sensors: 底层传入的真实传感器数值 (SensorData 类型)
        :return: 预测误差百分比 (0.0 - 100.0)
        """
        error_score = 0.0

        # 1. 物理环境温度偏离预期
        real_temp = sensors.temperature
        ...
        has_msg = sensors.has_new_message
        if has_msg != self.expected_user_active:
            error_score += 50.0  # 极大的惊喜/惊吓误差

        # 3. 网络异常断开
        is_online = sensors.is_network_online
        if not is_online:
