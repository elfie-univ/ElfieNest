...
class ExecutionScope(FrozenContractModel):
    """Durable authority envelope for one future Activity step.

    Unlike a Turn ``ResponseScope``, this scope survives across Turns and
    therefore records the resolved target, capability revision, permitted
    operation set, and expiry used by Preflight.
    """

    external_domain: Optional[ExternalExecutionDomain]
    target_actor_id: Optional[ActorId] = None
    ...
    allowed_operations: Tuple[_NonBlankText, ...] = Field(min_length=1)
    expires_at: UTCDateTime

    @model_validator(mode="after")
    def validate_target(self) -> ExecutionScope:
        if self.external_domain is ExternalExecutionDomain.COMMUNICATION:
            valid = (
                self.target_actor_id is not None
        ...
        if not valid:
            raise PydanticCustomError(
                "activity_execution_target",
                "Activity execution target does not match its external domain",
            )
        if len(set(self.allowed_operations)) != len(self.allowed_operations):
            raise PydanticCustomError(
                "activity_execution_operations",
                "Activity allowed operations must be unique",
            )
        return self

    def allows(self, operation: str) -> bool:
        """Return whether Preflight authorized one semantic operation."""
...
class ActivityStep(FrozenContractModel):
    """One bounded step with an explicit durable execution envelope."""
    ...
    def validate_scope(self) -> ActivityStep:
        ...
        if not self.scope.allows(self.operation):
            raise PydanticCustomError(
                "activity_operation_scope",
                "Activity operation is not permitted by its execution scope",
            )
        if self.deadline > self.scope.expires_at:
            raise PydanticCustomError(
                "activity_scope_expiry",
                "Activity step deadline exceeds its execution scope",
            )
        if self.kind is ActivityStepKind.COMMUNICATION:
            if self.scope.external_domain is not ExternalExecutionDomain.COMMUNICATION:
                raise PydanticCustomError(
            ...
            if self.scope.external_domain is not ExternalExecutionDomain.NERVOUS_SYSTEM:
                raise PydanticCustomError(
                    "activity_nervous_system_scope",
                    "nervous-system Activity steps require a body scope",
                )
        elif self.scope.external_domain is not None:
            raise PydanticCustomError(
...
def activity_scope_for_record(record: ActivityRecord) -> Optional[ResponseScope]:
    ...
    return None if step is None else step.scope.response_scope()
