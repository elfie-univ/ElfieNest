...
class EpisodeMemoryManager:
    ...
    def record_episode(self, event_description: str, emotion_tag: str = "calm", emotion_intensity: float = 0.0):
        """
        录入一条新发生的具体情景经历
        ...
        self.storage.add_memory(text=event_description, tags={"emotion": emotion_tag}, intensity=emotion_intensity)

    def retrieve_relevant_memories(self, query: str, level: str = None, current_emotion: str = None) -> List[str]:
        """检索出语义高度关联的情景经历"""
        return self.storage.retrieve_relevant_memories(query, top_k=2, level=level, current_emotion=current_emotion)
