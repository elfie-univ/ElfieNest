"""Bounded offline cognition for one quiet, sleeping Brain window.

Offline Cognition is deliberately a scheduler-facing Brain subsystem, not a
...
class OfflineCognitionCheckpoint:
    ...
    pending_candidate: Optional[OfflineCognitionCandidate]


class OfflineCognitionRestoreError(ValueError):
    """Raised when an offline-cognition checkpoint would rewind state."""


@dataclass(frozen=True)
class OfflineCognitionResult:
...
class OfflineCognitionSystem:
    ...
    def __init__(
        ...
        *,
        ...
        self._status: OfflineCognitionStatus = "ready"
        ...
        self._pending_candidate: Optional[OfflineCognitionCandidate] = None
        self._lock = RLock()
    ...
    def evaluate(
        ...
        *,
        ...
        with self._lock:
            ...
            self._revision += 1
            candidate_id = EventId(f"offline:consolidation:{self._revision}")
            self._last_trigger_id = candidate_id
            self._cooldown_until = now + timedelta(seconds=self._cooldown_seconds)
            self._status = "cooldown"
            self._pending_candidate = OfflineCognitionCandidate(
    ...
    def snapshot(self, captured_at: UTCDateTime) -> OfflineCognitionSnapshot:
        with self._lock:
    ...
    def checkpoint(self) -> OfflineCognitionCheckpoint:
        with self._lock:
            return OfflineCognitionCheckpoint(
                revision=self._revision,
...
def offline_candidate_to_perception(
    candidate: OfflineCognitionCandidate,
    *,
    elfie_id: ElfieId,
) -> PerceptionEvent:
    ...
    return PerceptionEvent(
        meta=MessageMeta(
            ...
            elfie_id=elfie_id,
            source=ActorRef(
                actor_id=ActorId(f"{elfie_id}:offline-cognition"),
                source_kind="offline_cognition",
            ),
            occurred_at=occurred_at,
            received_at=occurred_at,
            trace_id=TraceId(f"offline-cognition:{candidate.candidate_id}"),
        ...
        payload=InternalPayload(
            type="internal",
            signal=InternalSignal.OFFLINE_COGNITION,
...
__all__ = (
    ...
    "OfflineCognitionResult",
    "OfflineCognitionStatus",
