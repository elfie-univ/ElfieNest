...
from infrastructure.persistence.configuration.config_store import write_yaml_mapping
...
def _provider_secret_name(provider_id: str) -> str:
    profile = BUILTIN_PROFILES.get(provider_id)
    configured_name = profile.api_key_env_var if profile else ""
    return provider_secret_name(provider_id, configured_name=configured_name)
...
class RuntimeLab:
    ...
    def _food_store(self) -> SQLiteFoodAdapter:
        root = (
    ...
    def _show_provider_evidence(self, provider_id: str) -> None:
        evidence = [
            ...
            if item.reference.startswith(f"{provider_id}/")
        ]
        if not evidence:
        ...
        for item in evidence:
            ...
            label = item.display_name or item.reference
            ability = _format_capabilities(item.capabilities)
            self.output(
                f"- {status} {label} [{item.reference}]: {latency} "
    ...
    def _food_strategy_menu(self, store: SQLiteFoodAdapter) -> None:
        while True:
            catalog = store.load()
            items = []
    ...
    def _show_food_detail(self, store: SQLiteFoodAdapter, food_key: str) -> None:
        catalog = store.load()
    ...
    def _update_foods(self, store: SQLiteFoodAdapter) -> None:
        ...
        if not evidence:
            ...
            return
        current_packages = store.list_packages()
        planner = FoodPlanner()
        ...
        for package in current_packages:
            ...
            packages[package.food_id] = proposal.package
            changes.extend(
    ...
    @staticmethod
    def _food_recipe_diff(
        old_recipe: Any,
        new_recipe: Any,
    ) -> list[tuple[str, str, str]]:
        def values(package: Any) -> dict[str, str]:
            if package is None:
                return {}
            return {
    ...
    def _validate_foods(self, store: SQLiteFoodAdapter) -> None:
        catalog = store.load()
        ...
        refreshed: list[StoredModelEvidence] = []
        ...
        tool_recipe = next(
            (package for package in catalog.ordered_packages() if package.tool),
            None,
        )
        if tool_recipe and tool_recipe.tool is not None:
            ...
            if tool_model and tool_model.verified:
                ...
                record_model_evidence(
                    [
                        StoredModelEvidence(
                            reference=tool_model.reference,
    ...
    def _create_custom_provider(self) -> bool:
        ...
        provider: dict[str, Any] = {
            ...
            "api_key_env": _provider_secret_name(provider_id),
            "status": "active",
        }
        if api_key:
            provider["api_key"] = api_key
        ...
        if self.config_home:
            ...
            if pending_secret is not None:
                set_provider_secret(
                    ...
                    pending_secret,
                    self._secret_file_path(),
                    configured_name=_provider_secret_name(provider_id),
                )
        else:
    ...
    def _verify_model_agent(self) -> None:
        available = sorted(
            ...
            key=lambda item: (item.display_name or item.reference, item.reference),
        )
        if not available:
            self.output(
            ...
            return
        model_by_key: dict[str, StoredModelEvidence] = {}
        items = [
        ...
        for index, item in enumerate(available, 2):
            ...
            label = item.display_name or item.reference
            items.append(MenuItem(key, label, item.reference))
        choice = self.menu.choose(
        ...
        for index, selected in enumerate(selected_models, 1):
            provider, model = selected.reference.split("/", 1)
            self.output(
                f"\n[{index}/{len(selected_models)}] "
                f"Validating {selected.display_name or selected.reference}…"
            )
