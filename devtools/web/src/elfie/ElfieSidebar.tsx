...
function ExperimentConfig({ foods, food, warning, onFood, onConfigureFood }: Readonly<{ foods: readonly FoodItem[]; food: string; warning: string; onFood: (key: string) => void; onConfigureFood: () => void }>): React.JSX.Element {
  const selected = foods.find((item) => item.key === food) ?? foods[0];
  return <details className="experiment-section"><summary><span>实验配置</span><i>＋</i></summary><div className="model-controls"><label htmlFor="foodSelect">本次实验粮食</label><select disabled={foods.length === 0} id="foodSelect" onChange={(event: ChangeEvent<HTMLSelectElement>) => onFood(event.target.value)} value={selected?.key ?? ""}>{foods.map((item) => <option disabled={!item.ready_for_attempt} key={item.key} value={item.key}>{item.display_name}{item.ready_for_attempt ? "" : " · 尚未配置"}</option>)}</select><p className={selected?.ready_for_attempt ? "is-ready" : "is-error"}>{selected ? `${selected.model || "未配置模型"} · ${selected.description || "粮食尚未配置"}` : "暂无可用粮食"}</p>{warning ? <p className="is-error">{warning}</p> : null}<button className="secondary-button food-configure-button" onClick={onConfigureFood} type="button">{selected?.ready_for_attempt ? "重新配置测试粮食" : "配置测试粮食"}</button></div></details>;
}
