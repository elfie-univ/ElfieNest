...
function Preview({ iframeRef, preview, status }: Readonly<{ iframeRef: Props["iframeRef"]; preview: Preview; status: string }>): React.JSX.Element {
  ...
  return <section className="appearance-section"><div className="section-heading"><div><span>外貌</span><strong>3D 个体视图</strong></div><small>{status}</small></div><div className="appearance-viewport"><iframe onLoad={() => setLoaded(true)} ref={iframeRef} src="/godot-web/elfienest.html?mode=elfie_lab" title="当前精灵 3D 外貌" /></div><div className="appearance-tools"><button aria-label="向左旋转" onClick={() => preview("orbit", { delta: { x: -0.28, y: 0 } })} type="button">↶</button><button aria-label="向右旋转" onClick={() => preview("orbit", { delta: { x: 0.28, y: 0 } })} type="button">↷</button><button aria-label="缩小" onClick={() => preview("zoom", { delta: 0.18 })} type="button">−</button><button aria-label="放大" onClick={() => preview("zoom", { delta: -0.18 })} type="button">＋</button><button aria-label="复位视角" onClick={() => preview("reset")} type="button">⌂</button><button aria-label="头部取景" onClick={() => preview("focus", { target: "head" })} type="button">◉</button><button className="capture-button" onClick={() => preview("capture")} type="button"><span>拍照</span><b>◎</b></button></div></section>;
}
...
function Switcher(props: Readonly<Pick<Props, "items" | "session" | "collapsed" | "menuOpen" | "portraitEpoch" | "onCreate" | "onDelete" | "onMenu" | "onSelect">>): React.JSX.Element {
  ...
  return <div className="elfie-switcher-wrap" ref={wrapRef}><div className={props.collapsed ? "elfie-menu compact-switcher-menu" : "elfie-menu"} hidden={!props.menuOpen} role="menu">{props.items.map((item) => props.collapsed ? <button aria-label={`切换至${item.name}`} className={item.elfie_id === session.elfie_id ? "active" : ""} key={item.elfie_id} onClick={() => props.onSelect(item.elfie_id)} role="menuitem" type="button"><Portrait compact elfieId={item.elfie_id} epoch={props.portraitEpoch} name={item.name} url={item.portrait_url} /></button> : <div className="elfie-menu-row" key={item.elfie_id}><button className={item.elfie_id === session.elfie_id ? "active" : ""} onClick={() => props.onSelect(item.elfie_id)} role="menuitem" type="button"><Portrait compact elfieId={item.elfie_id} epoch={props.portraitEpoch} name={item.name} url={item.portrait_url} /><span>{item.name} · {item.species_id === "dog" ? "小狗" : "狐狸"}</span></button><button aria-label={`删除${item.name}`} onClick={() => props.onDelete(item.elfie_id)} type="button">⌫</button></div>)}{props.collapsed ? null : <><hr /><button onClick={props.onCreate} type="button">＋ 新建测试精灵</button></>}</div><button aria-label="切换测试精灵" className="elfie-switcher" aria-expanded={props.menuOpen} aria-haspopup="menu" onClick={props.onMenu} type="button"><Portrait compact elfieId={session.elfie_id} epoch={props.portraitEpoch} name={session.profile.name} url={session.profile.portrait_url} /><span><strong>{session.profile.name}</strong><small>切换测试精灵</small></span><b>⌃</b></button></div>;
}
