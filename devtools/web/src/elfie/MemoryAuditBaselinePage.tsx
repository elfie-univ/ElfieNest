...
export function MemoryAuditBaselinePage(): React.JSX.Element {
  ...
  const graph = useMemo(() => {
    ...
    const assertions = report.data.assertions.filter((item) => (Array.isArray(item.evidence_ids) ? item.evidence_ids : []).some((id) => ids.has(String(id))));
  ...
  return <main className="memory-audit-page memory-audit-baseline-page">
    ...
    <section className="memory-audit-shell"><div className="memory-audit-main">
      ...
      <div className="memory-audit-baseline-cards">{(report?.data.episodes ?? []).map((episode) => <button className={String(episode.episode_id) === episodeId ? "is-active" : ""} key={String(episode.episode_id)} onClick={() => { setEpisodeId(String(episode.episode_id)); setSelectedEdge(null); }}>{short(String(episode.summary_text ?? episode.content_text ?? episode.episode_id), 30)}</button>)}</div>
