...
class ElfieNestCoordinator:
    ...
    def trigger_elfie_interaction(self, sender_id: str, receiver_id: str, event_type: str):
        """
        兼容 main.py 接口：模拟一个物理碰撞/揉尾巴事件，
        将该刺激投递到对应精灵的具身触觉缓冲区中，以在下一个 Tick 激活脑干反射！
        """
        logger.info(f"💥 [物理刺激] 触发来自 '{sender_id}' 针对 '{receiver_id}' 的 {event_type} 交互")
        ...
        if event_type == "collision":
            ...
            self.api_server.send_action("physical_impact_event", {
                "elfie_id": receiver_id,
                "impact_type": "gentle_stroke"
            })
...
class ElfieNestEngine:
    ...
    """
    def __init__(self, ws_host: str = "127.0.0.1", ws_port: int = 8765, http_port: int = 8000):
        # 1. 实例化核心组件
        ...
        self.temp_audio_dir = os.path.abspath(os.path.join(os.getcwd(), "assets", "temp"))
        ...
        self.api_server.register_callback("register_scene", self._on_godot_scene_registered)
    ...
    def _start_http_server(self):
        ...
        try:
            ...
            self._http_thread = threading.Thread(
                target=self.httpd.serve_forever, daemon=True, name="ElfieNest_HTTP_Thread"
            )
            self._http_thread.start()
            logger.info(f"🎵 [语音服务] 静态音频分发服务器已在 http://127.0.0.1:{self.http_port} 成功挂载！映射目录: {self.temp_audio_dir}")
    ...
    async def _async_generate_tts(self, text: str, output_path: str, voice: str = "zh-CN-XiaoxiaoNeural"):
    ...
    def _synthesize_voice(self, elfie_id: str, text: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"🎤 [语音服务] 精灵 '{elfie_id}' 发言音频合成成功 -> {audio_url}")
            return audio_url
        except Exception as e:
            logger.warning(f"⚠️ [语音服务] edge-tts 合成失败 (可能是网络超时或包未完全安装)，优雅降级为空音频: {e}")
            return None

    def start_loop(self, runtime_agent: Any, ticks_to_run: int = 3, interval_sec: float = 1.5):
        """
        启动世界物理 Tick 仿真循环。
        ...
        logger.info(f"⏳ [时间盒子] 物理仿真启动。总计运行 {ticks_to_run} 个 Tick 周期，每个周期阻尼间歇 {interval_sec} 秒...")
        ...
        try:
            while current_tick < ticks_to_run:
                ...
                for elfie_id, elfie in list(self.room.elfies.items()):
                    ...
                    if not status.get("active", True) or status.get("posture") == "away":
                        continue
                    ...
                    logger.info(f"👀 [具身感知] 精灵 '{elfie_id}' 正在感知环境: {raw_sensor_data}")
                    
                    # 2. 激活大脑神经冲动闭环 (脑干反射弧检测 -> 丘脑组装 Context -> 皮层 LLM 决策)
                    response = elfie.perceive_and_respond(raw_sensor_data, runtime_agent)
                    ...
                    if response.get("success", False):
                        ...
                        logger.info(f"💬 [具身响应] 精灵 '{elfie_id}' 发言: \"{speech_text}\" (碎碎念: {mutter})")
                        logger.info(f"🏃 [具身响应] 精灵 '{elfie_id}' 执行物理意图: {action}")
                        ...
                        if speech_text:
                            ...
                            self.api_server.send_action("speak_event", {
                                "elfie_id": elfie_id,
                                "text": speech_text,
                        ...
                        # 6. 转译并下发物理语义动作
                        if action and action != "reflex_avoidance" and action != "reflex_soothing":
                            # 将大模型动作映射到语义家具目标
                            ...
                            if "sleep" in action_lower or "bed" in action_lower:
                                target_furniture = "bed_1"  # 默认床 1，后续可实现动态选空闲床
                                posture = "lying"
                                animation = "sleep_loop"
                            elif "sit" in action_lower or "chair" in action_lower or "chat" in action_lower:
                                target_furniture = "chair_1" # 默认椅子 1，后续可实现动态选空闲椅
                                posture = "sitting"
                                animation = "chat_look"
                            elif "door" in action_lower or "away" in action_lower or "leave" in action_lower:
                            ...
                            if target_furniture:
                                # 更新房间被动意向状态
                                self.room.update_elfie_posture(elfie_id, f"moving_to_{target_furniture}", target_furniture)
                                
                                # 下发给 Godot 去进行 3D 寻路与寻路到达 Area 的碰撞反馈
                                self.api_server.send_action("go_to", {
                                    "elfie_id": elfie_id,
                        ...
                        logger.warning(f"⚠️ [时钟驱动] 精灵 '{elfie_id}' 心智决策略过: {response.get('reason')}")
