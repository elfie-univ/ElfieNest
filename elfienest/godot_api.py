...
class GodotAPIServer:
    ...
    def register_callback(self, event_name: str, callback: Callable[[Dict[str, Any]], None]):
        """注册针对 Godot 事件的监听回调"""
    ...
    def start(self):
        ...
        self._thread = threading.Thread(target=self._run_event_loop, daemon=True, name="ElfieNest_WS_Thread")
        self._thread.start()
        ...
        while self._loop is None or not self._loop.is_running():
            ...
            if time.time() - t0 > 3.0:
                ...
                break
        logger.info(f"🚀 [通信网关] 后台通信线程就绪，正在 {self.host}:{self.port} 监听连接...")
    ...
    async def _handle_client(self, websocket: websockets.WebSocketServerProtocol, path: str):
        ...
        logger.info(f"🤝 [通信网关] 收到来自 Godot 的连接握手: {websocket.remote_address}")
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    if event_name:
                        logger.info(f"📥 [通信网关] 接收到 Godot 事件: {event_name} - {payload}")
                        self._trigger_callbacks(event_name, payload)
                    else:
                        logger.warning(f"⚠️ [通信网关] 收到无 event 标签的非法消息: {message}")
                except json.JSONDecodeError:
                    logger.error(f"❌ [通信网关] 消息解析 JSON 失败: {message}")
        except websockets.exceptions.ConnectionClosed as e:
            logger.info(f"👋 [通信网关] Godot 连接断开: {websocket.remote_address} (code={e.code})")
        finally:
    ...
    def _trigger_callbacks(self, event_name: str, payload: Dict[str, Any]):
        ...
        for cb in callbacks:
            try:
                ...
                logger.error(f"❌ [通信网关] 执行回调 '{cb.__name__}' 抛出异常: {e}", exc_info=True)
    ...
    def send_action(self, action: str, payload: Dict[str, Any]):
        ...
        # 在 asyncio 线程安全地派发任务
        asyncio.run_coroutine_threadsafe(self._async_broadcast(message_dict), self._loop)
