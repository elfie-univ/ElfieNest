...
class ElfieNestRoom:
    ...
    def register_scene_furniture(self, furniture_list: List[str]):
        ...
        logger.info(f"🧱 [房间容器] 动态加载 Godot 场景成功，已注册语义化家具看板: {list(self.room_state['furniture'].keys())}")
        ...
        for elfie_id in self.elfies:
            if elfie_id != sender_id:
                ...
                if status.get("active", True) and status.get("posture") != "away":
                    ...
                    logger.info(f"📣 [群聊广播] 已将 '{sender_id}' 的发言暂存到 '{elfie_id}' 的感知缓冲中")
    ...
    def update_elfie_posture(self, elfie_id: str, posture: str, target_furniture: str = None):
        """
        更新精灵的动作姿态和家具绑定状态
        """
        ...
        self.room_state["elfies_status"][elfie_id]["target_furniture"] = target_furniture
        logger.info(f"🔄 [房间状态] 精灵 '{elfie_id}' 状态更新为 姿势={posture}, 绑定家具={target_furniture}")
