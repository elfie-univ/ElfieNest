...
func _render() -> void:
	...
	var actor_scenes := SPECIES_CATALOG.discover_actor_scenes()
	if candidate_id.is_empty() or not actor_scenes.has(species_id):
	...
	if not appearance is Dictionary:
		...
		quit(2)
		return
	var scene := actor_scenes[species_id] as PackedScene
	if scene == null:
...
func _render_view(
	...
	if actor == null:
		viewport.queue_free()
		await process_frame
		push_error("portrait scene root is not a CharacterBody3D")
		return null
	actor.set("install_shared_animations", false)
	world.add_child(actor)
	actor.call("configure", identity, Vector3.ZERO, appearance)
	actor.set_physics_process(false)
	await process_frame
	await process_frame
	await process_frame
	var bounds: AABB = actor.call("visual_bounds")
	if headshot:
