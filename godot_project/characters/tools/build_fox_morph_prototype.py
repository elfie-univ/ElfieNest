...
def group_weight(
    ...
    indices = {
        mesh.vertex_groups[name].index for name in names if name in mesh.vertex_groups
    }
    return min(
        1.0, sum(item.weight for item in vertex.groups if item.group in indices) * 2.0
    )
...
def add_shape_key(
    ...
    mesh.data.update()
    print(
        f"MORPH_CREATED name={name} affected={affected} max_delta={maximum_delta:.5f}"
    )
...
def make_arm_deformer(
    ...
    segments = [
        (
            bone_head_in_world(mesh, armature, start),
            bone_head_in_world(mesh, armature, end),
        )
    ...
    for variant, weights in variants:
        ...
        for view_name, (target, location) in views.items():
            ...
            bpy.context.scene.render.filepath = str(
                output_dir / f"{variant}__{view_name}.png"
            )
...
def main() -> None:
    ...
    if not meshes or len(armatures) != 1:
        raise RuntimeError(
            f"狐狸结构异常：mesh={len(meshes)} armature={len(armatures)}"
        )
    ...
    add_shape_key(
        mesh, "Body_ArmThickness_Neg", make_arm_deformer(mesh, armature, -1.0)
    )
    ...
    print(
        f"MORPH_PROTOTYPE_DONE blend={blend_path} glb={glb_path} previews={preview_dir}"
    )
