...
Chromium BrowserWindow 中加载已经导出的 Godot Web Runtime；不读取 `godot_project/`
源项目，也不要求运行用户安装 Godot。

Godot 编辑器和 Export Templates 只属于构建环境：

- 修改 `godot_project/` 场景、脚本、模型或材质的开发者需要安装。
- CI 发布构建机需要安装。
...
| 内容 | 路径 |
| --- | --- |
| Godot 源项目 | `godot_project/` |
| Web 导出预设 | `godot_project/export_presets.cfg` |
