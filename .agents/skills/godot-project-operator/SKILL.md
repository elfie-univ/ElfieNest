...
2. Godot 专项门禁的验证主体只允许单次、同步、headless 验证。若项目首次运行或
   生成缓存缺失，受控 `validate` 会先串行执行一次 Godot 自带的
   `--headless --import` 预热，再执行这一次验证；禁止使用 `open -n`、启动交互式编辑器、
   启动游戏或并行启动实例。
...
6. 仅在没有当前项目或归属不明的 Godot 进程时运行一次 headless 验证，并等待预热（如需要）
   和验证进程完全退出。其他项目的进程不阻塞验证；不得后台启动、自动重试或把临时进程留在后台。
