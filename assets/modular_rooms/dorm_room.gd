...
const BED_SCENE := preload("res://room/bedroom/base_bed.tscn")
...
func _build_entryway() -> void:
	...
	G.add_box(
		_generated,
		"DormDoorTrack",
		Vector3(0.09, 0.07, DOOR_WIDTH + 0.16),
		Vector3(-D.DORM_DEPTH / 2.0 + 0.07, DOOR_HEIGHT - 0.12, 0.0),
		Color("#a9977d")
	)
	G.add_box(
		_generated,
		"DormDoorHeaderTrim",
		Vector3(0.04, 0.045, DOOR_WIDTH + 0.22),
		Vector3(-D.DORM_DEPTH / 2.0 + 0.11, DOOR_HEIGHT - 0.02, 0.0),
		Color("#f1eadf")
	)
