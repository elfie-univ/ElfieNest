...
PROVIDER_METADATA = {
    ...
    "deepseek": {
        ...
        "cheap": {"model": "deepseek-chat", "desc": "DeepSeek V3 (极佳性价比，极强中文)"},
        "deep": {"model": "deepseek-reasoner", "desc": "DeepSeek R1 (深度思考与超强逻辑)"},
        "multimodal": {"model": "deepseek-chat", "desc": "DeepSeek V3 (无原生多模态，以 Chat 替代)"}
    },
    "gemini": {
        "name": "Google Gemini",
        "api_base": "https://generativelanguage.googleapis.com/v1beta",
        "test_model": "gemini-1.5-flash",
        "cheap": {"model": "gemini-1.5-flash", "desc": "Gemini 1.5 Flash (大上下文，高性价比)"},
        "deep": {"model": "gemini-1.5-pro", "desc": "Gemini 1.5 Pro (殿堂级推理，超长记忆)"},
        "multimodal": {"model": "gemini-1.5-pro", "desc": "Gemini 1.5 Pro (极致多模态，原生听觉视觉)"}
    },
    "qwen": {
        "name": "阿里通义千问 (DashScope)",
        "api_base": "https://dashscope.aliyuncs.com/compatible-mode/v1",
        "test_model": "qwen-coder-turbo",
        "cheap": {"model": "qwen-coder-turbo", "desc": "Qwen-Coder-Turbo (高能效代码日常)"},
        "deep": {"model": "qwen-coder-plus", "desc": "Qwen-Coder-Plus (强推理专业级代码)"},
        "multimodal": {"model": "qwen-vl-plus", "desc": "Qwen-VL-Plus (阿里原生高画质视觉模型)"}
    },
    "ollama": {
        "name": "本地 Ollama (完全离线)",
        ...
        "multimodal": {"model": "moondream", "desc": "Moondream 2 (本地轻量多模态视觉)"}
    }
}

def render_progress_bar(completed: int, total: int, prefix: str = "", suffix: str = "", length: int = 40):
    """在终端渲染精美的流式进度条"""
    if total <= 0:
        return
        ...
        with urllib.request.urlopen(req) as response:
            ...
            with open(OLLAMA_PATH, "wb") as f:
                while True:
                    ...
                    f.write(buffer)
                    render_progress_bar(downloaded, total_size, prefix="📥 下载底座", suffix="正在写入...")
    ...
    try:
        with urllib.request.urlopen(req) as response:
            for line in response:
                ...
                if total > 0:
                    render_progress_bar(completed, total, prefix=f"📦 {model_name}", suffix=f"[{status}]")
                else:
...
def setup_code_plan_interactive():
    ...
    # 初始化配置容器
    providers = {
        "deepseek": {"api_key": "", "api_base": PROVIDER_METADATA["deepseek"]["api_base"]},
    ...
    while True:
        ...
        if selected_provider == "ollama":
            ...
            custom_base = input(f"   请输入 Ollama 监听主机地址 [回车默认: {providers['ollama']['api_base']}]: ").strip()
            if custom_base:
                providers["ollama"]["api_base"] = custom_base
            continue
            
        print(f"\n🔐 正在配置服务商: {meta['name']}")
        key = input(f"   请输入 API Key (空值代表不启用): ").strip()
        if not key:
        ...
        if not success:
            ignore = input("   ⚠️  物理连通测试未通过。是否强制保留该配置？(y/n) [默认 n]: ").strip().lower()
            if ignore != 'y':
                print("❌ 已放弃保存该无效配置。")
                continue
    ...
    # 收集当前已激活的服务商
    active_providers = [k for k, v in providers.items() if k == "ollama" or v["api_key"]]
    ...
    for tier in ["cheap", "deep", "multimodal"]:
        ...
        selected_idx = input(f"请输入选项编号 [默认 1]: ").strip()
        if not selected_idx:
            selected_idx = "1"
            
        if not selected_idx.isdigit() or int(selected_idx) < 1 or int(selected_idx) > len(candidate_list) + 1:
        ...
        if idx_val == len(candidate_list) + 1:
            # 用户自定义输入
            print("   【手动自定义模型绑定】")
            prov_input = input("   请输入该模型的归属 Provider (如 openai, deepseek, qwen, ollama): ").strip().lower()
            model_input = input("   请输入该模型的真实大模型名称 (如 gpt-4o-2024-11-20): ").strip()
            
            if not prov_input or not model_input:
                print("❌ 输入不可为空！被迫退回并默认绑定第 1 项推荐。")
                routing[tier] = {"model": candidate_list[0]["model"], "provider": candidate_list[0]["provider"]}
            else:
        ...
        print(f"🎯 成功绑定【{tier} 档】 ➡️ 模型: '{routing[tier]['model']}' (Provider: {routing[tier]['provider']})")
    ...
    try:
        ...
        print("   - 【Cheap 档】: {} ({})".format(final_config['cheap_model'], final_config['cheap_provider']))
        print("   - 【Deep  档】: {} ({})".format(final_config['deep_model'], final_config['deep_provider']))
        print("   - 【Multi 档】: {} ({})".format(final_config['multimodal_model'], final_config['multimodal_provider']))
...
def main():
    ...
    if not ollama_exec:
        print("❌ 本地未检测到已安装的 Ollama 二进制，且非 macOS 系统无法自动托管下载。")
    ...
    if not service_already_running:
        ...
        try:
            ...
            for _ in range(15):
                ...
                print("❌ 算力服务自启动超时！请检查本地是否被防火墙阻拦，或手动运行 'ollama serve'。")
                if process:
                    process.terminate()
                sys.exit(1)
        except Exception as e:
