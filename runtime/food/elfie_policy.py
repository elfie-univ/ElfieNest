...
from runtime.food.models import FIXED_FOOD_KINDS, FoodValidationStatus
...
class ElfieFoodPolicy:
    ...
    allowed_foods: tuple[str, ...] = ("coarse", "standard")
    ...
    def from_dict(cls, elfie_id: str, data: Mapping[str, Any]) -> ElfieFoodPolicy:
        allowed = tuple(
            str(item)
            for item in data.get("allowed_foods", ("coarse", "standard"))
            if str(item) in FIXED_FOOD_KINDS
        )
        if not allowed:
            allowed = ("coarse", "standard")
...
def resolve_food_selection(
    ...
    if requested not in policy.allowed_foods:
        actual = _first_available(
            (policy.default_food, policy.fallback_food, *policy.allowed_foods), catalog
        )
...
def _food_available(food_key: str, catalog: FoodCatalog) -> bool:
    ...
    return bool(
        ...
        and recipe.validation_status is not FoodValidationStatus.FAILED
    )
