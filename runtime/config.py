...
class LLMRuntimeConfig:
    ...
    providers: Dict[str, Dict[str, str]] = field(default_factory=lambda: {
        "deepseek": {
            "api_key": os.getenv("DEEPSEEK_API_KEY", ""),
            "api_base": os.getenv("DEEPSEEK_API_BASE", PROVIDER_RECOMMENDS["deepseek"]["api_base"])
        },
        "openai": {
            "api_key": os.getenv("OPENAI_API_KEY", ""),
            "api_base": os.getenv("OPENAI_API_BASE", PROVIDER_RECOMMENDS["openai"]["api_base"])
        },
        "gemini": {
            "api_key": os.getenv("GEMINI_API_KEY", ""),
            "api_base": os.getenv("GEMINI_API_BASE", PROVIDER_RECOMMENDS["gemini"]["api_base"])
        },
        "qwen": {
            "api_key": os.getenv("QWEN_API_KEY", ""),
            "api_base": os.getenv("QWEN_API_BASE", PROVIDER_RECOMMENDS["qwen"]["api_base"])
        },
        "ollama": {
            "api_key": "",
            "api_base": os.getenv("OLLAMA_HOST", PROVIDER_RECOMMENDS["ollama"]["api_base"])
        }
    })
    ...
    # 4. 路由与能耗控制阈值
    energy_threshold_fast: float = 30.0    # 物理精力低于 30% 强制降级使用 cheap 模型以省钱/省电
    ...
    def __post_init__(self):
        ...
        if os.path.exists(json_path):
            try:
                ...
                if "providers" in saved_cfg and isinstance(saved_cfg["providers"], dict):
