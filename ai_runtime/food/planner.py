...
class FoodPlanner:
    ...
    def propose(
        ...
        generation_note = (
            "Generated by model suggestions and rule validation"
            if self._advisor_status == "used"
            else "Planning model unavailable, generated using rules"
            if self._advisor_status == "failed"
            else "No planning model configured, generated using rules"
        )
    ...
    def _select_model(
        ...
        if food_key == "emergency":
            # Emergency food is the last resort when network/cloud are unavailable.
...
def validate_food_recipe(
    ...
    if not recipe.primary.model:
        return ["Food not configured with main model, please auto-update food strategy"]
    ...
    if recipe.key == "emergency" and not selected.local:
        warnings.append("Emergency food main model must be a validated local model")
