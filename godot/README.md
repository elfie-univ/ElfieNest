...
`godot/` 是 ElfieNest 3D 世界的唯一源码来源，负责房屋、几何、坐标、移动、
碰撞、角色和渲染。Python Core 可以交换巢内事件与状态，但不得复制这里的场景
布局或空间事实。
...
不要把 `godot/` 当作普通脚本目录直接批量格式化，也不要把编辑器生成物当作
源码提交。
...
发布打包再把同一份产物复制到单 target staging；不要在 `godot/`、`desktop/`
或普通用户 Web 源码中维护副本。
