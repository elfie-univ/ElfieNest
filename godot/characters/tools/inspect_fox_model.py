...
def main() -> None:
    ...
    output_dir = root / "godot/characters/fox/source/previews/neutral"
    ...
    if not meshes or len(armatures) != 1:
        raise RuntimeError(f"预期至少一个网格和一个骨架，实际为 {len(meshes)} / {len(armatures)}")
    ...
    print(f"FOX_BOUNDS min={tuple(round(v, 5) for v in minimum)} max={tuple(round(v, 5) for v in maximum)}")
    ...
    blend_path = root / "godot/characters/fox/source/fox_neutral_import.blend"
