...
| 现有模块 | 调整方向 |
|---|---|
| `elfienest/adoption/generator.py` | 领养编排留在 Nest；领域生成迁入 `elfie/profile` |
| `elfienest/adoption/service.py` | 接收 `AdoptionIntent`、生成候选、确认并事务化保存 |
| `elfie/brain/cognition/profile.py` | 改为读取类型化 `ElfieProfile`，不直接拼松散 YAML |
...
| `elfie/brain/cognition/brain.py` | 移除“艾菲、小狐狸、傲娇”等固定提示词 |
| `elfienest/simulation/engine.py` | 从 resolver 获取稳定 Godot 载荷，不解析 YAML metadata |
| `godot/main.gd` | 必须使用明确 `species_id`，迁移完成后删除 ID 哈希兜底 |
| `elfie/nervous_system/*` | 动作、语音和反射文案读取个体身份与表达投影 |
| `elfienest/api/user_routes.py` | 删除任意 YAML 覆盖接口，改用类型化档案读取和受限更新 |
