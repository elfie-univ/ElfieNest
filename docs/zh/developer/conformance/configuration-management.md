...
| CFG-003 | P0 | closed | Desktop staging 只把仓库 `config/` 复制一次到 `resources/config/`；发行 manifest 覆盖每个 staging 文件，用户配置不会被复制或覆盖；发行态解析必须由 launcher 提供资源根。 | 保持单份 staging、完整哈希、脱离源码 checkout 启动和用户文件保留。 | target=single-staged-bundled-root; inventory=资源组装、manifest 必需路径/哈希、发行态 resolver 与首次运行/用户写入行为；references=`scripts/internal/build/assemble_desktop_resources.py`、`scripts/release_manifest.py`、`test/scripts/test_assemble_desktop_resources.py`、`test/scripts/test_release_manifest.py`; verification=组装、manifest、安装根与用户保留测试；residuals=zero |
