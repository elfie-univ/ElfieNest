...
| MEM-008 | P0 | 阻塞 | 确定性结构门禁已通过，但真实 Ark 候选/裁判运行和负责人体验复核尚未完成。 | 可重放脱敏报告必须在 Stage 1 晋级前证明结构门禁、来源锚定、关系/冲突、重启和延迟。 | target=设计第 9.7 节及 `docs/.internal/elfie-stage1-memory-backed-chat-execution-plan.md`；inventory=`devtools/evals/stage1_chat_ark.py`、场景集和聚焦测试；references=E1 预检/阻塞报告；verification=332 项组合受影响测试、持久化扫描退出码 0、代表性夹具纯数据库 p95 约 40.6 ms；residuals=已获授权的真实 Ark 运行仍在模型调用前被本机 ArkCLI 临时目录权限拦截；人工复核仍待完成。 | 阻塞尝试未产生外部模型调用，也未泄露密钥。 |

## 剩余验收顺序

1. 修复本机 ArkCLI 临时目录权限后，按已获授权的范围仅使用合成场景重跑真实 Ark；评测器保持密钥本地并只写入脱敏证据。
2. 完成人工体验复核并记录晋级决定。
