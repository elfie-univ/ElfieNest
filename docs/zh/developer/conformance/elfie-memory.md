...
> 状态：当前分支实现已完成；外部模型验收和生产切换仍受明确门禁控制<br>
> 基线：2026-08-27<br>
...
| --- | --- | --- | --- | --- | --- | --- |
| MEM-001 | P0 | 已关闭 | 目标 Adapter 已用审查后的 Episode、节点、Assertion 和 Evidence 表替代旧实体/子类型及嵌入边布局。 | 目标表、约束、所有权和可重建词法投影已实现，不存在第二套边事实源。 | target=设计第 9.1–9.2 节；inventory=`infrastructure/persistence/memory/schema.py`、`node_store.py`、`edge_store.py`；references=持久化扫描；verification=目标 schema/往返/重开测试及 315 项受影响测试；residuals=已有线上旧库仍需下方的显式导入/切换。 | 开发目标无残余。 |
...
| MEM-008 | P0 | 阻塞 | 确定性结构门禁已通过，但真实 Ark 候选/裁判运行和负责人体验复核尚未完成。 | 可重放脱敏报告必须在 Stage 1 晋级前证明结构门禁、来源锚定、关系/冲突、重启和延迟。 | target=设计第 9.7 节及 `docs/.internal/elfie-stage1-memory-backed-chat-execution-plan.md`；inventory=`devtools/evals/stage1_chat_ark.py`、场景集和聚焦测试；references=E1 预检/阻塞报告；verification=315 项受影响测试、37 项架构测试、持久化扫描退出码 0、代表性夹具纯数据库 p95 约 40.6 ms；residuals=真实 Ark 调用被本机 ArkCLI 临时目录权限拦截，且向外部模型发送合成提示需要明确授权；人工复核仍待完成。 | 阻塞尝试未产生外部模型调用，也未泄露密钥。 |

## 剩余验收顺序

1. 如需完成体验验收，明确授权仅使用合成场景运行真实 Ark；评测器保持密钥本地并只写入脱敏证据。
2. 完成人工体验复核并记录晋级决定。
...
修改 schema 后必须再次运行。每一行只有在 target、inventory、references、verification 和 residuals 五类信息都记录完整后才能关闭。MEM-008 在外部模型和人工验收完成前保持阻塞。
