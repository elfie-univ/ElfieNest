...
> 状态：Memory 实现和本轮审查强化已完成；确定性门和真实 Ark 门均通过，负责人已确认匿名体验样本。本轮强化的聚焦验证已全绿；生产数据切换仍是 MEM-007 下的独立操作；OPT-003/004 按计划暂缓<br>
> 基线：2026-08-27<br>
...
| MEM-005 | P0 | closed | `RecallRequest` 已执行确定性的 Basic/Text 候选检索，再做有界 Local Graph 遍历、来源获取、关系/时间/facet/隐私过滤和限制控制。active、superseded 及冲突声明保留状态和证据。 | 文本覆盖罕见/未解析表述；图遍历覆盖明确关系；来源和冲突保持可见。 | target=设计第 6、9.4 节；inventory=`sqlite_retrieval_store.py`、`node_store.py`、`sqlite_graph_store.py`；references=source-first 检索测试；verification=罕见词/别名、人物关系网、知识对象、种子、时间窗口、正向 AND/OR facet、未知时间、隐私、跳数/限制和代表性延迟检查；residuals=Global/社区和向量检索仍是后续投影。 | 当前词法投影有意保持简单且可重建。 |
...
| OPT-003 | P1 | deferred | 有界 Lifecycle/Memory Maintenance 实现已经存在，但长期压缩、遗忘、归档、增长和延迟仍未做耐久性验证。 | 在启用无人值守长期维护前，建立有界增长、重试/恢复和生命周期 p95 评测。 | target=设计第 6、9.2/9.6 节；inventory=`sqlite_lifecycle_store.py`、`memory_system.py`；references=Memory 合同强化测试；verification=有界维护、来源保护和幂等衰减测试通过；residuals=耐久性/10k Episode 容量证据仍待补齐。 |
...
修改 schema 后必须再次运行。每一行只有在 target、inventory、references、verification 和 residuals 五类信息都记录完整后才能关闭。本轮 Memory 强化对开发目标已收口；生产切换和 OPT-003 长期容量证据仍单独治理。
