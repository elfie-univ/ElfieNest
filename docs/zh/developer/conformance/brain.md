...
| BRN-002 | P0 | open | `BrainContext` 已接入带来源、版本、未知字段和当前回合 cutoff 的 Orientation、Selfhood、不可变 Profile 锚点及版本化 Memory 状态投影；但 Motivation、Activity 仍没有权威快照，既有 owner-memory 兼容写入仍在。 | 上下文组装无隐式写入地读取全部版本化所有者快照，区分事实/推测/未知，并记录一个 Turn 使用的全部快照 cutoff。 |
