...
| ELF-003 | P0 | open | Brain 使用宽泛历史 `CorticalRuntimePort`，Food 选择、模型访问和工具行为尚未成为最终窄 Port。 | Brain 分别拥有强类型 `FoodPort`、`ModelPort`、`ToolPort`；Food 保留命名角色/单 Fallback/Emergency 行为，限定作用域的 Tool Adapter 保留技术安全否决权；Bootstrap 注入 Infrastructure View；迁移全部调用方并删除宽 Runtime 桥。 |
