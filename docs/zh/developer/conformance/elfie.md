...
| ELF-018 | P0 | open | 三个 Brain 域和动态能力目录链路已经实现；虚拟 Godot 移动/身体回传切片已在第一阶段 Brain-owned Mock 模式下真实房间验证。 | 保持恰好 `Communication`/`Embodied`/`Activity`；`ACCEPTED`/`STARTED` 只留在账本；通过 EventWorkspace 发布一个具身终态和兼容身体事实；补齐并单独取证模型驱动控制，以及听觉/视觉/触觉/位置场景。 | target=ADR-0033 与 Brain/Elfie/System/Nest-Godot 1.6/2.4/1.10/1.2 契约；inventory=Brain workspace/决定类型、Body/NervousSystem、Godot Adapter/Transport/Gateway 与执行计划；references=动态能力目录、域化回执和 Brain-owned Mock 控制器；verification=受影响 Python 套件 349/349 通过；真实 Godot 房间 E2E `build/e2e/brain-godot-live` 有 `world_ready`/`intent_terminal`；compile/lint/架构检查；residuals=外部物理身体、第二版异步提交/回执流以及模型驱动/实时感知验收仍不在本切片 |
