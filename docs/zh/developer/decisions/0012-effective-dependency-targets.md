...
- 解析静态有界的 Python 加载器间接形式，包括字典导出注册表、元组/列表解包、索引
  查找和包相对模块名，避免 lazy export 隐藏仓库依赖边；
- 将无法解析的 Python 动态模块加载参数显式报告为质量门禁失败，而不是把缺少证据当成
  已获准；
...
对于静态有界的注册表，Python resolver 采取保守策略：当运行时 key 不能在扫描时选定，
就检查注册表中的全部字面量值，而不是把这次查找静默视为外部依赖。超出这种有界形式的
动态表达式会直接阻断有效依赖门禁，必须改为窄 Port 或 Bootstrap-owned 计划，或经过
单独审查的治理规则覆盖。

Node 基于 URL 的产物加载以及 TypeScript 只存在于类型层的 `import()` 表达式，暂不纳入
这个“未解析参数即失败”的规则，直到 parser 能区分这些非仓库目标和运行时仓库模块加载；
字面量仓库目标仍继续使用现有 Node 检查。
