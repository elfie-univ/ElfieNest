...
- 数据根就是任务身份。安装版使用 `${ELFIE_HOME:-~/.elfienest}`，不保存生产选择，也没有
  `data-home` 命令；源码模式忽略调用方 `ELFIE_HOME`，只有 `start`、`serve`、`restart`、
  `stop` 允许 `--data-home`，上下文只存在内存中。
- `web`、`mobile`、`desktop` 只打开已有健康目标。端口只是证据，不是身份或清理目标；类型化
  失败必须保留在 generation 快照和解析后的数据根日志中。
