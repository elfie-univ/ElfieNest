...
普通优化循环优先使用引导式界面，不必一开始就运行完整批量协议：

```bash
./developer.sh elfie-lab
```

1. 在左侧创建或选择一只合成测试精灵，并配置一份可运行的 Food；
2. 从**单次实验**切换到**版本评测**；
3. 小改动后选**快速检查**（3 个场景），版本节点选**标准评测**（8 个场景）；
4. 选择自动评审 Food 并运行，然后查看进度、P0 红线、每个场景的旧基线/当前输出、Q6 维度变化、
   Brain 延迟和 Brain 模型调用；
5. 第一份拥有可用证据且完成的运行会成为本地开发基线，以后相同 Fixture 的运行自动与它比较。
   只有人工看过证据后，才使用**设为新的开发基线**。

开发基线只是比较原点，不是“本轮全部通过”的证书；其中未通过或证据不足的场景会原样保留。
...
Lab 运行与基线保存在
`${ELFIE_DEV_HOME:-~/.elfienest-dev}/elfie_lab/evaluations/<elfie-id>/`。删除测试精灵时，评测历史
会一起进入 Lab 回收包；进程异常退出后，下次启动会把未结束任务明确标成“证据未完成”，而不会
永久停在“运行中”。候选身份绑定 Git revision、每一次不同的未提交工作区快照和所选 Food/模型；
运行也保存 Judge Food/模型配置摘要。
