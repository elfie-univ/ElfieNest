...
### 1.1 在 Elfie Lab 做日常版本评测

普通优化循环使用报告工作区，不必一开始就运行正式晋级协议：

```bash
./developer.sh brain-eval
```

1. 先在**单精灵实验**中保存模型订阅，再从订阅提供的模型里创建一份或多份可运行 Food。模型订阅
   是共享的连接记录，Food 是引用它的独立角色配置；第一次真实调用才尝试连接；
2. 切换到独立的全屏**批量评测**页面。默认页面是一张全局报告表，不保留单精灵侧栏和 3D 面板；
3. 用**新建单次评测**冻结当前精灵并生成一份事实报告；或用**新建配对评测**只改变 Food 或代码；
4. 小改动选**快速检查**（3 个场景），版本节点选**标准评测**（8 个场景）。单次评测和粮食配置配对
   评测使用当前分支的最新代码；代码配对评测在同一行分别选择两个分支，后台解析并记录各自最新
   提交。候选侧选择一套粮食配置；单独的**评审模型**一行从同一份共享订阅目录中选择一个具体模型。
   评审模型不创建粮食、不读取粮食，也不参与候选运行；评审选择器只展示远程 HTTPS 订阅，新增时仍
   保存到同一份订阅记录，后续可被 Food 复用。保存前用所选模型做一次连接验证；本地 Ollama/本机地址
   不允许作为评审模型。
   这里不会额外增加一套“模型健康检查”流程；
5. 单击一份报告，在宽右抽屉查看快照、候选方案、测试计划、结果和证据；任意勾选两份报告即可
   查看对比。配对父行会把 A/B 子报告一起选中，并打开同一份对比抽屉。

一份报告是一轮运行的不可变事实证据；配对批次只是两份报告之间的关联。对比产物与两份报告的
内容哈希一起持久化，反复打开抽屉不会静默重复调用 Judge。

对比固定分为三级：

| 等级 | 条件 | 允许的结论 |
| --- | --- | --- |
| 严格配对 | 精灵快照、测试计划和 Judge 相同，且只有 Food 或代码一个变量不同 | 可以把观察到的变化归因给该变量 |
| 多变量观察 | Fixture/计划相同，但多个候选变量或 Judge 配置不同 | 可以展示变化，不能声称因果 |
| 条件不兼容 | 精灵快照或测试计划不同 | 只能并排看证据，不输出胜负 |

Food 配对只冻结一次快照，再克隆给两个候选。代码配对也只冻结一次快照、测试计划、Food 和 Judge，
然后分别从用户选择的两个代码分支运行；报告展示分支名，后台同时记录解析后的提交和内容摘要。
历史报告仍可在报告页任意两份比较，但不会被新建代码配对偷偷复用或重跑。Lab 不会假装能够直接
执行任意历史 Git SHA。

快速检查覆盖通信/身体 Scope、身份锚点和重启记忆；标准评测包含这些场景，并用不知道时澄清、
关系/隐私、情绪分寸、安静时间克制和承诺 Preflight 等案例覆盖全部六个 Q6 维度。

当前 Lab 套件不会启动或要求 Godot 评测场景。它们检查对话、类型化 Decision、Receipt、隔离记忆、
虚拟时间和会话重建。以后若要声称移动、碰撞、导航或其他物理事实正确，才需要增加由 Godot
权威状态和回执支持的场景 Adapter；这不是当前 3/8 场景预设的一部分。

报告保存在 `${ELFIE_DEV_HOME:-~/.elfienest-dev}/elfie_lab/evaluations/`。报告 JSON、批次关联、
对比产物和冻结快照分开保存。快照通过 SQLite 在线备份复制所选测试精灵的档案、记忆、活动、
Brain 日志和受支持的当前状态；Provider 密钥绝不进入报告或快照。每个场景都从同一快照的新副本
开始。异常退出后未结束报告会明确失败；配对只完成一边时会标成“部分失败”，不输出胜负。

单份报告中的软质量场景在没有确定性绝对标准时显示**证据已就绪**，而不是“通过”；Q6 的方向性
结论只在相对比较中产生。延迟和模型调用数仍是资源证据，不会折算成质量分。当前契约只有
`-1 / 0 / +1` 的比较方向时，界面不会伪造 0–100 总分。

这些结果明确属于探索反馈：单次运行没有置信区间，Lab Judge 尚未通过人工锚点校准，相同被测/
评审模型会给出警告，并且任何 Lab 结论都不能自动晋级代码。需要支撑正式晋级时，再使用下文的
批量工作流。

`catalog` 中的“24 个家族”只是内部参数化覆盖模板，不是 24 项产品功能、Brain 组件或必须逐个
点击的 UI 动作。快速检查和标准评测只是从这套内部分类中选出的第一批可运行产品预设。
