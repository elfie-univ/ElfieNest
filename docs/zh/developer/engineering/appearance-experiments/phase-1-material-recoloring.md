...
这些图片保留的是当时选择 V9 视觉方向的历史证据。正式实现现在直接在动画3D材质表面执行同一套
相对明暗迁移，不在渲染图片上后处理。冻结的13区证据由正式区域渲染器单独维护。
...
第一阶段作为两个物种的视觉基线通过。正式实现已经采用可复用的“源贴图换色”路径，并按物种
配置源颜色锚点与特征保护。被否决的大范围覆盖、截图换色和 UV Mask 实现继续保持删除。
