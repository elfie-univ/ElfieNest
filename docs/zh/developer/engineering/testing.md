...
拆为已注册测试包：`architecture`、`devtools`、`e2e`、`elfie`、`godot`、`infrastructure`、
`nest`、`scripts`，以及 App 的 Bootstrap、各 Feature/Configuration 子域、API/CLI/Web Interface、
各 Orchestration 子域、跨切面 Orchestration 测试和产品 E2E 模块。它只运行缺失或失效的包；
包输入包含测试和共享 `conftest.py` 的本地 Python 传递 import，只有通过记录、覆盖率片段摘要/版本、
可读覆盖率数据和不可变基础提交都一致时才能复用。动态或非 Python 入口必须显式列为输入。覆盖率
片段使用相对路径；包含绝对路径的片段会被拒绝，因此内容完全一致的产物可以在候选树或其他
Worktree 中安全复用，不会混入旧源码路径。同一次运行共享仓库快照，并在命中前复核签名，最后
合并全部片段并只执行一次全仓覆盖率阈值。一个包失败后，此前通过的包记录仍然保留；下次调用会
跳过它们，从失败或尚未运行的包继续。
...
如果 `--selectors` 命中一个已注册测试包，或命中某个模块包所包含的子目录，它会产生 G3
使用的同一份带覆盖率证据。混合选择或更窄的 node/单文件结果只能按原聚焦命令复用，不能
证明无关的完整包。原始 `pytest` 只用于诊断，不产生提交缓存证据。
