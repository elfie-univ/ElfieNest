...
不可变的 Genesis 资料注册表是“哪些物种可以被创建”的唯一发行 authority，但源资料发布
与生产激活是两回事。
...
物种只有属于当前唯一已激活的创建资料包，并且创建规则、人生可行性及运行资产均校验通过，
才可用于领养。仅发布源资料条目不能开放领养。经单路径切换验收后，已启用且合格的物种
无需管理员写入配置或按巢批准，就可供已有安装使用。该变更只影响未来候选和创建，
不能刷新既有 Elfie 的 Profile、Selfhood 或 Memory。
...
验收不变量是：设置不变时，已发布但未激活的物种不出现在领养列表；完成激活及逐物种
校验后，合格物种才由 `GET /api/v1/me/adoption` 列出且能够生成候选。运行角色仍为
草稿等不完整物种不得开放。
...
只有最终 owner 产物可重开、Adoption 归属/额度和 Nest 居民/床位都对同一预约确认后，
入驻结果才对产品可见。仅发布文件树或写入领养关系不算完成。Runtime 注册在提交后进行，
可独立恢复；未完成的 Admission 不得出现在正式居民视图中。
