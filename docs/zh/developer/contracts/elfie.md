...
唯一创建路径有五道语义门：证明人生可行后生成最多五位不同候选；接受一位并冻结身份、
外貌、人格和资料包/策略版本；不改锚点地构造时间与路线均合法的 `LifeContext`；只生成
本人有资格掌握的完整知识、实际人物/经历和闭集映射的 Selfhood；最后联合校验并列产物，
再由 Admission 发布。后一步不能编造前一步的条件，也不能重抽已接受事实。地理、学习和
通行均来自同一已激活资料包；未知事实保持未知，不由模型或默认值填补。
