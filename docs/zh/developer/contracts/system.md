...
> 子契约实现缺口记录在各自已注册一致性文件中。永久 Scanner 和架构测试执行本目标中
> 当前可机器检查的部分。
...
[配置管理契约](./configuration-management)细化非 Python 内置默认配置根、用户配置根、
...
`config/` 是顶级非 Python 源码资源根，只保存配置管理契约注册的内置声明式默认值；
它不是第五个生产代码所有权模块，也不是可写运行时目录。
...
这些方向同时约束有效依赖和 import。通过模块名、可执行脚本、subprocess、子进程、
Shell 命令或动态加载器启动仓库模块，会形成从调用方所有者到目标所有者的依赖边。
把禁止模块名从 import 移到命令字符串，不会改变或隐藏这条边。Developer Tools 可以
通过产品公开边界进行隔离实验；生产根和产品入口脚本不得以 `devtools/` 为目标。
...
Infrastructure，也不自行执行 SQL。记忆算法、语义模型角色选择、Skill 文档加载与 Tool
allow-list、身体命令和感知模型仍属于 Elfie。Skill 加载边界由
`elfie/brain/reasoning/skill_port.py` 拥有，可执行 Tool 定义由 Infrastructure Tool
注册表拥有。
...
| 单次认知步骤的工具选择 | `elfie/` Reasoning Core Tool policy | `infrastructure/tools/` 执行获批准的受限请求 | Elfie 消费强类型结果；App 配置全局可用性 |
...
Elfie Reasoning 通过只读 Catalog 从内置 `config/brain/skills/<name>/SKILL.md` 加载获批的
流程 Skill。这些文档不是可执行 Tool，也不授予 Tool 权限；Brain 的 Reasoning Core 保留
强类型加载边界和 Tool policy。App Feature 拥有管理员可见的全局启用和配置。搜索、文件、代码或设备执行实现属于 `infrastructure/tools/`，并继续
受工具安全和有界结果契约约束。

已注册的应用级声明式默认值，包括当前 Brain、物种和 Nest 文档，统一位于根
`config/`，并且只 staging 一次到 `resources/config/`。其语义模型仍归对应 App、Elfie、
Nest、Models 或 Tools 所有者；产品和领域代码只接收强类型值，不解析路径或直接读取
文件。算法不变量和少量启动安全常量可以保留在代码中，但不能复制已注册产品默认值。
...
4. 更新全部生产调用方；
5. 删除被替代的旧技术实现和兼容路径；
6. 用聚焦证据和端到端证据证明完整调用链。

所有代码都必须遵守本契约。永久系统 Scanner 不使用旧债基线，直接以 deny-all 模式
运行。未来任何临时缺口都必须遵守仓库治理契约，不能隐式削弱本契约。
