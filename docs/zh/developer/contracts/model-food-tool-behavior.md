...
> **拆解期间的行为权威。** 本文保留已退役 `ai_runtime/` 根目录中已经接受的
> Provider、模型、Food 与工具行为，但不定义目标 Runtime 模块。目标所有权、依赖和
> 物理位置由[系统架构契约](system)统一控制。当前偏差记录在
> [Model/Food/Tool 一致性台账](../conformance/model-food-tool-conformance)。
...
只有这条闭环通过，并且一致性台账没有任何 open 条目，实现才算符合设计。
