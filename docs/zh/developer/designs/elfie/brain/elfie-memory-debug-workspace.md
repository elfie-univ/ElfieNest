...
- 顶部操作栏左侧聚合搜索、筛选和记忆操作，右侧放置图谱操作，中间保留弹性间隔；
- 搜索和筛选相邻但职责分开：搜索执行 Recall，筛选打开当前图谱的条件面板；添加 Episode 与手动 Consolidation 保留为两个独立操作；
- 搜索框与搜索结果面板保持相同的桌面宽度；筛选按钮紧邻搜索框，顶部操作组仍靠左排列；搜索使用统一组件的清除交互，不手绘输入框或清除按钮；
- Episode 时间带保持在顶栏下方的原位置并可横向滚动；左右弹层仍叠在图谱上，左侧操作面板只向右错开到首张 Episode 卡片之后并留出间距，仍留在左侧，不移到画布中间；右侧详情面板仍在右侧；
- 添加 Episode 使用加号图标，手动 Consolidation 使用触发操作图标；
- 中间图谱继续占据工作区画布；
- 左侧可关闭弹层承载搜索结果，或添加 Episode 及其处理结果；
- 右侧可关闭弹层只显示选中对象的详情；
- 点击左侧结果后，中间图谱高亮对应对象，右侧打开详情，左侧结果列表继续保留；
- 左右弹层可以分别关闭，选择、过滤和高亮状态保持联动；
...
- 让中央图谱、左侧操作面板和右侧详情保持同一个选择、过滤和高亮上下文。
...
图谱铺满工作区。顶栏左侧依次放置搜索/筛选与记忆操作，搜索框与搜索结果面板保持相同的桌面宽度；顶栏右侧放置图谱视图操作，两组之间使用弹性间隔。搜索执行 Recall，筛选打开当前图谱的条件面板；添加 Episode 与手动 Consolidation 是不同操作，分别保留入口。

Episode 来源带保持在顶栏下方的原位置并可横向滚动。左右面板仍是叠加在图谱上的可关闭弹层，不预留固定栏宽，也不挪到 Episode 来源带下方：左侧操作面板只向右错开到首张 Episode 卡片之后，不移到画布中间；右侧详情面板保持在右侧：

- 左侧弹层：由顶部现有的搜索和添加 Episode 操作打开，内容在 Search/Recall 与 Add Episode 两种模式间切换；
- 右侧弹层：只显示当前选中对象的详情；
- 两个弹层分别显示 × 关闭按钮，开关互不影响；桌面工作区是目标，不设计窄屏抽屉布局。

图谱负责“看整体结构、来源和受影响范围”。左侧弹层负责“搜索或运行 Episode 操作”，右侧弹层负责“看选中对象的完整事实”。

### 4.2 左侧结构

左侧从上到下包含三个显示区域：

1. Episode 来源带：按时间或当前操作上下文排列可见 Episode，支持横向滚动。Episode 是来源层，不与下方知识图中的 Node 重复；左侧操作面板打开时，首张 Episode 卡片保持可见，时间带仍留在顶栏下方原位。
...
详情面板由图谱或左侧结果选择驱动，不自行产生第二份数据。点击左侧结果后，左侧列表保持打开，右侧显示对应详情。
...
每一步都必须有可追溯的 operation id、输入版本、输出版本和观测状态。中央图谱按步骤高亮新建、更新、受影响和被跳过的对象；左侧显示处理过程和结果。
...
6. 最终 RecallBundle；
7. RecallBundle 中返回的 Episode、Node、Assertion、Evidence，以及对应的图谱高亮。未评分节点需标出“未评分”；零分节点不作为节点相关命中，但作为命中 Assertion 端点时仍显示关系高亮。

左侧必须允许逐候选展开，不能只显示“返回了 N 条”。中央图谱默认保留全库：强调正相关度返回节点、未评分但确实返回的节点，以及命中关系的端点；零分节点和被排除候选不得伪装成节点相关命中。零分节点若同时是命中关系端点，仍按关系命中显示。用户可显式切换为仅显示搜索结果子图。搜索时 Episode 来源带只列出命中的 Episode。点击结果后，右侧显示该对象详情，左侧结果列表保持打开。
...
- 添加 Episode 的每一步更新中央图谱中的受影响对象；
- 搜索决定更新 returned Node 和命中 Assertion 端点的高亮；零分 Node 与被排除候选不作为节点相关命中高亮；零分 Node 若同时是命中 Assertion 的端点，则仍按关系命中显示；
- 清除搜索会清除查询与 Recall 高亮并关闭左侧搜索面板，已打开的右侧详情保持不变；
- 清除选择只清除选择，不清除过滤；
...
| 理解 Episode 到知识 | 9/10 | 来源、Evidence、Node、Assertion 分层，左侧显示操作结果，中央图谱显示影响范围 |
| 解释一次 Recall | 9/10 | 候选级评分、淘汰原因、预算裁剪与关联高亮明确分工 |
| 定位当前数据问题 | 9/10 | 详情、证据、生命周期、冲突、版本和失败步骤可追溯 |
| 左右联动效率 | 9/10 | 左侧搜索/操作、中央图谱和右侧详情共享选择状态 |
