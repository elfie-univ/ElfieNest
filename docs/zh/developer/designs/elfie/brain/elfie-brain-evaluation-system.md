...
> 人工锚点校准、聚类统计、受约束晋级，以及 Elfie Lab 中基于报告的批量评测界面均已实现

> 设计关系：**所属模块：**Elfie / Brain / Evaluation；**上级设计：**[Brain 十系统架构](./elfie-brain-ten-system-architecture.md)；
> **下级设计：**无；**规范性契约：**[Brain 契约](../../../contracts/brain.md)；**当前架构：**[Brain 评价工作流](../../../engineering/brain-evaluation.md)；
> **一致性台账：**[Brain 一致性](../../../conformance/)；**领域资料来源：**Product 评价目标与 Brain 测试夹具。
...
| 层级 | 适用场景 | 结论权限 |
| --- | --- | --- |
| Elfie Lab **批量评测** | 普通 Brain 改动后的快速反馈 | 探索性单份报告与报告间比较 |
...
一次执行 -> 一份不可变报告
一份或两份报告 -> 一个批次行
任意两份已完成报告 -> 一个绑定双方内容哈希的对比产物
```

一份报告冻结六部分。它们不是随意凑出的六张卡，而是共同回答“究竟运行了什么、控制了哪些条件、
留下了什么证据”：

| 部分 | 冻结含义 |
| --- | --- |
| 评测对象 | 所选合成测试精灵的身份与稳定档案 |
| Fixture 快照 | 一次冻结的档案、记忆、活动、Brain 日志和受支持的当前状态 |
| 候选方案 | 源码提交与未提交内容摘要、所选 Food、实际模型引用和配置摘要 |
| 测试计划 | 面向产品的快速/标准场景集，以及顺序、重置、重复和种子规则 |
| Judge 规格 | 只用于软质量相对判断的独立远程评审订阅、模型和配置摘要 |
| 结果与证据 | 场景输出、类型化事实、P0、Q6 方向、错误、延迟和模型调用 |

因此，“样本集”和“执行规则”属于同一个测试计划：只有问话而没有重置、顺序和种子规则，结果不可
复现；只有规则而没有场景内容，也不是可用测试。Food 属于候选方案，不属于 Fixture。Judge 单独
记录，因为候选行为完全相同时，换 Judge 仍可能改变解释。

全局表格中，单次评测是一行独立报告；配对评测是一个可展开父行，下面放 A/B 两份子报告。勾选
配对父行会一起选中两个子报告。“勾选比较”和“打开查看”是两套独立状态：打开某份报告不会破坏
已经选好的两份比较篮子。宽右抽屉分为单份报告状态，以及带**对比总览 / 报告 A / 报告 B**标签
的双份状态。

这种模型也明确了复用边界。事实报告不会因为被选中观察就重跑。新建严格 Food 配对时，两个候选
从同一份新快照分别运行；新建代码配对也从同一份新快照、测试计划、Food 和 Judge 分别运行两个
用户选择的代码分支，并在报告中显示分支名、后台保存解析后的提交。历史报告仍可在报告页任意
两份比较，但不会被新建代码配对隐式复用。纯展示 Diff 可以重新计算；Judge 得出的对比结论则
必须按两份不可变报告的内容哈希持久化。
...
| Elfie Lab 快速/标准批量报告、冻结 Fixture、配对运行、全局历史和宽证据抽屉 | 已作为探索反馈实现；绝不输出正式晋级 |
