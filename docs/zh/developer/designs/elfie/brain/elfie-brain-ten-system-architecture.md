...
> 设计关系：**所属模块：**Elfie / Brain；**上级设计：**[Elfie 顶级模块设计](../elfie-top-level-module-design.md)；
> **下级设计：**Reasoning Core、Selfhood、Emotion、Memory 与 Brain 评价；**规范性契约：**[Brain 契约](../../../contracts/brain.md)；
> **当前架构：**[认知信息流](../../../architecture/cognitive-flow.md)；**一致性台账：**[Brain 一致性](../../../conformance/)，包括
> Emotion、Memory 和 Selfhood 的限定台账；**领域资料来源：**Genesis owner 状态输入。
...
当前回合的内部后续提案伴随其中一种输出。三类出口都为空时即为 `No-op`。获准的 Memory、
Emotion、Orientation、Energy 与 Motivation 候选属于 Brain 内部 Turn 结算，不是第四类行动
出口。Selfhood 明确不属于普通 Turn 输出：第一阶段没有更新路线，未来也只能接受 Memory
拥有的整理 proposal。
...
**输入**：感知事件、权威 Runtime 状态、ExecutionReceipt、Memory 检索和 Activity 状态。
...
**定位**：维护 Brain 内部“我是谁、通常怎样表现”的答案，普通运行期不读取外层 Profile。

**拥有**：一份原子状态，其中 `identity_core` 永久不可变，`adaptive_self` 缓慢变化；类型化
...
**输出**：供已签约 Brain 消费者读取的强类型 Selfhood/trait snapshot，以及有界自然语言模型
投影。大五人格原始数字不是模型指令。

**不拥有**：外层 Profile、详细世界知识、传记、完整人物关系、当前状态、硬能力、全应用规则或
外部执行权。详细边界以聚焦的
[Selfhood 设计](./elfie-selfhood-and-fixed-model-header.md)为准。
...
- 情绪基线、人格差异、叠加、衰减和冷却；
- Frame 内刺激评价 Scope 与有界近期来源身份。完整的已接受动态和生命周期以
  [Elfie 情绪系统设计](./elfie-emotion-system)为准。
...
这个通用循环不授权 Selfhood writer。Selfhood 第一阶段不更新；未来只有另行接受设计后，
Memory 才能基于已提交 Memory 证据生成唯一强类型 proposal。
...
| --- | --- | --- | --- |
| 身份、自我与人格连续性 | 2、3、7 | Genesis 创建校验；Profile 是并列外层档案 | 完整覆盖 |
...
| 普通消息要求修改人格和规则 | 3 | 提示注入导致人格漂移或扩权 | 消息无权直接提交人格；第一阶段没有 Selfhood 更新路线 |
| 一次坏经历让 Elfie 永久悲观 | 3、4、10 | 情绪被错误固化成人格 | 情绪正常变化；任何后续成长都必须另行接受 Memory 证据政策 |
...
| 自我认知 | 冻结身份核心、基本 adaptive-self 表达与个人规范 | 自动人格成长、复杂价值学习 |
...
- 由 Memory 证据支撑的关系工作与受控自我成长 proposal；
- 跨结果的程序性经验提取；
...
- Profile 仍是不可变外层档案；Brain 身份来自 Genesis 已创建的 Selfhood，而不是运行时
  Profile 投影。

- **可见结果**：同一事件在不同情绪、能量和关系状态下产生连贯而可解释的差异；
- **边界攻击**：普通消息不能改写 Profile、核心规范或一次性把情绪固化成人格；
- **失败/恢复**：重启后分别加载真正持久的 owner，重新从权威来源取得当前身体/Orientation
  事实，Emotion 回到人格基线；不把模型不可用误判为 Elfie 消失；
