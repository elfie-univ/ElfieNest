...
> [Elfie 顶级模块设计](../../zh/developer/designs/elfie/elfie-top-level-module-design.md)为准，Brain 的当前概念架构以
> [Elfie 大脑十系统架构](../../zh/developer/designs/elfie/brain/elfie-brain-ten-system-architecture.md)为准
...
ElfieNest 的虚拟生活世界由三个功能核心和一个上层组织者共同构成。最上层判断只回答四个问题，
不把功能继续打碎到实现细节：

| 部分 | 核心职责 | 回答的问题 |
| --- | --- | --- |
| Godot | 物理世界、虚拟身体、动作执行与空间观察 | 物理上能不能发生，实际发生了什么？ |
| Nest | 空间与设施、巢内生活规则、时间与环境、精灵与巢交互，以及横切的巢内事件机制 | 这个家里有什么、遵守什么规则、环境怎样变化，精灵怎样获得带生活含义的感知与行动结果？ |
| Elfie | 同一个生命的意图、认知、感知、情绪、记忆与反应 | 我为什么想做，感受到什么，如何回应？ |
| App | 组合、同步、路由、生命周期与故障恢复 | 怎样让前三者完成一次正确且可恢复的流程？ |

这四部分遵守六条总规则：

1. 一个事实只有一个权威，其他部分只能保存带来源和版本的投影；
2. Godot 负责连续、高频、依赖空间的物理计算，Nest 不复制坐标、碰撞、导航或声学空间；
3. Nest 负责长期、共享、需要规则判断的生活语义，Godot 不保存居民归属、家庭规则或精灵认知；
4. Elfie 发出高层意图并消费感知和回执，不逐帧控制身体，也不把模型输出当成已经发生的事实；
5. Elfie 是自身身体行为的唯一意图发起者；Nest 可以在一次已授权语义请求中解析并转发动作，也可以
   自主控制环境对象，但不能脱离 Elfie intent 自主决定或改写 Elfie 身体行为；
6. App 只协调 authority，不创造第四套物理事实、生活规则或角色智能。

虚拟世界中的说话采用语义捷径，不模拟一条没有产品价值的“文字转语音、播放、再语音转文字”
链路。Elfie 向 Nest 发起带文本和**表露情绪**的巢内说话请求；Nest 暂存本次发言的语义内容，并让
Godot 根据说话模式、角色位置、墙体、门和空间规则计算哪些虚拟身体处于可听范围；Nest 再把可达
结果与原始内容关联，应用居民有效性与巢内传播规则，最后把类型化听觉感知投递给对应 Elfie。
真实音频若存在，只用于人类观察者的可选呈现，不参与精灵是否听见的判定。表露情绪是可观察的
语气或外在表达，不能泄露说话者未表达的内部真实情绪。

虚拟视觉同样采用语义捷径。MVP 不为每只 Elfie 渲染主观摄像头截图，也不调用视觉模型；Godot 根据
actor 朝向、距离、视野与遮挡产生一次性的可见实体集合，Nest 只补充“我的床、公共椅子、厨房入口”
等家庭语义，再把结构化视觉感知定向交给对应 Elfie。Nest 不保存动态视野，Godot 也不理解家庭归属。

“回自己的床、去厨房、找可用杯子”等语义身体行为也不拆成两次认知。Elfie 只发出一次高层 intent，
Nest 在该 intent 授权内解析语义目标和生活规则，Godot 完成寻路与物理执行，Nest 再关联语义结果；
执行中的触觉和本体感知仍由 Godot 直接返回 Elfie。

Nest 内部最终分为空间与设施、巢内生活规则、时间与环境、精灵与巢交互四个一级功能模块。视觉、
听觉和语义行动统一由“精灵与巢交互”组织；四个模块各自产生自己拥有的语义事件，公共事件机制负责
因果身份、受众解析后的唯一投递和幂等。广播只是 NestEvent 的一种受众范围，不是所有 Runtime 事件
的默认处理方式，也不是第五个业务模块。

Nest、Godot 的详细功能模块、接口面和协调过程见
[Nest 与 Godot 虚拟生活世界最终设计](../../developer/designs/nest-godot-virtual-world-functional-architecture.md)；
可执行的所有权与事件边界以
[Nest–Godot 语义世界契约](../../developer/contracts/nest-godot-semantic-world.md)为准。
...
flowchart TB
    ...
    NestCore["Nest<br/>四个功能模块 + 公共事件机制"]
    Brain -->|"生活查询、语义行为、主动观察与说话"| NestCore
    NestCore -->|"查询结果与内部规则触发"| Brain
    NestCore -->|"结构化视觉与 HeardUtterance"| Nervous
    NestCore -->|"已解析 Elfie 请求与环境对象命令"| VirtualBody["Godot 虚拟躯体与虚拟世界"]
    VirtualBody -->|"语义动作结果、VisibleSet、环境事实与 SpeechReach"| NestCore
    Body <-->|"明确目标动作、直接回执、触觉与本体感知"| VirtualBody
...
失败后删除。完整边界以
[Elfie 顶级模块设计](../../zh/developer/designs/elfie/elfie-top-level-module-design.md)为准。
