...
> 状态：已确认的内部架构基线
> 确认日期：2026-08-11
> 性质：目标目录和模块所有权规约，不代表当前源码已经完成迁移
> 暂不包含：模块通信协议、事件 Schema、调用时序和迁移实施方案

## 1. 规约目的

本文固定“一只完整 Elfie”在目标架构中的顶级目录、一级模块、数据所有权和禁止边界。
...
Profile 保存这只 Elfie 创建后不再变化的固有档案。它是一级模块，但不是持续运行的循环系统。
...
Memory；母亲是谁可能不会改变，但仍然属于 Relationship。Profile 只保存 Elfie 自身的固有身份、
虚拟外貌和创建来源。
...
`genesis/` 保存生命初始化的领域规则。它只在领养创建阶段运行，完成后不继续拥有生成出来的数据。
...
| 生成产物 | 最终所有者 |
| --- | --- |
| 固有身份、虚拟外貌、生成来源 | Profile |
| 初始人格 | Brain / Personality |
| 不超过 5 个关键领养前事件 | Brain / Memory |
| 初始人物关系 | Brain / Relationship |
| 初始自我认知 | Brain / Self Model |
| 后续历史补全边界 | Brain / Offline Cognition 的临时受限 Activity |
| 创建版本、完成状态和校验结果 | Initialization Manifest |
...
### 4.1 Genesis 与 Night Work 的边界

Genesis 必须在第一次醒来前完成最小完整人格：Profile、初始人格、关系骨架、Self Model 和不超过
5 个关键历史事件。

Genesis 可以产生临时 `BiographyEnrichmentPlan`。前几个 Night Work 周期只允许在该计划限定的
事件根、人物、事实、预算和失效时间内补充感官、情绪和关联细节。所有补充先成为 MemoryCandidate，
由 Memory System 审核后提交。

Night Work 不得修改 Profile、增加无限重大历史、发明新的关键亲友，或把故事补全变成永久后台
编剧。计划完成或到期后，该特殊补全活动终止。
