# Genesis 核心内核设计 v0.3：资料准备与个体生成的统一设计

> 状态：内部最终完整设计草案；受 ADR-0033 与 Elfie 2.3 / Brain 1.5 契约约束
>
> 版本定位：本文把总体设计讨论稿中的“资料准备流程”和“个体生成流程”与 Genesis 核心内核的
> 技术阶段合并为一套完整流程，是当前唯一活动的 Genesis 设计草案。v0.1、v0.2、总体讨论稿和
> 各执行计划只保留为历史输入或执行记录，不再定义平行流程。本文仍位于 Draft，不表示当前代码
> 已经实现，也不授权修改既有 Elfie 数据。
>
> 上游世界资料：
...
Genesis 由两个相互衔接、但不混用编号的流程组成。资料准备流程负责形成可反复使用的世界与居民知识
资料；个体生成流程负责把一次领养输入和这些资料编译成一个具体 Elfie。两者合起来才是完整的
“产生精灵并产生初始记忆”的过程。

### 0.1 资料准备流程

资料准备不是每次创建 Elfie 都重新进行，而是在世界资料或生成规则变更时重新发布版本化输入：

| 步骤 | 任务 | 主要产物 |
| --- | --- | --- |
| 步骤一：编写世界设定 | 从造物者视角确定世界的客观规律、环境、生物、物种、社会和地点 | 世界设定 Markdown，即 `CreatorWorldSkeleton` 的人读上游来源 |
| 步骤二：编写居民知识 | 从居民可观察、可传播、可学习的视角整理世界；明确知道什么、如何知道和认识边界 | 居民知识 Markdown，即 `ResidentKnowledgeBaseline` 的人读来源 |
| 步骤三：生成配置资料 | 将前两步整理为机器可读且可校验的 Genesis 输入 | `GenesisSourcePackage`，由 A 类构造/生成规则和 B 类居民知识资产组成 |

```text
世界设定 Markdown ──→ 居民知识 Markdown
       │                       │
       └──→ A．构造与生成规则   └──→ B．居民知识资产
                    ↓
...
A 类资料描述年龄、地点、生活路径、人物/关系、经历等结构化生成条件和规则；B 类资料描述居民实际
能够观察、传播、学习和表达的知识。A、B 是职责划分，不要求恰好对应两个文件；A 可以引用 B，
但不能复制一套独立居民百科。Markdown 是人读来源，YAML 等机器资料只能是语义对应的发布产物，
不能在机器配置中另行创作第二套知识。

### 0.2 个体生成流程

个体生成以用户已经接受的领养选择和已发布的 `GenesisSourcePackage` 为输入，按以下三步完成：

| 步骤 | 任务 | 主要产物 |
| --- | --- | --- |
| 步骤一：生成个体基础与生活背景 | 解析已接受的用户选择，确定身份、年龄、出生/生活地点、时间线和生活路径等后续条件 | `LifeContext` 及其创建期保留的 `AcceptedGenesisReservation`、`GenesisCompileEnvelope` |
| 步骤二：生成个人记忆 | 在 `LifeContext` 约束下，结合 A 类生成规则和 B 类居民知识，生成个体实际接触、掌握或经历的知识、人物关系和事件 | `PersonalGenesisPlan` |
| 步骤三：校验并完成生成 | 检查世界一致性、个人一致性、知识边界、所有权和提交完整性，一次性发布最终值 | `GenesisBundle`、`GenesisCommitReceipt`，写入 `Profile` / `Selfhood` / `Memory` |

```text
用户领养输入 + A 类生成规则
             ↓
步骤一：个体基础与生活背景（LifeContext）
             ↓
步骤二：个人记忆（知识、关系、经历）
       ↑                     ↑
       A 类生成规则          B 类居民知识资产
...
总体流程中的“三步 + 三步”是便于领域讨论和审查的视角；内核实现仍按六个技术阶段落地，不能因此
形成第二条流水线：

| 领域流程 | 技术阶段 |
| --- | --- |
| 资料准备步骤一：世界设定 | 阶段 0：`CreatorWorldSkeleton` |
| 资料准备步骤二：居民知识 | 阶段 1：`ResidentKnowledgeBaseline` |
| 资料准备步骤三：配置资料 | 阶段 2：发布 `GenesisSourcePackage` |
| 个体生成步骤一：基础与生活背景 | 阶段 3：`LifeContext` |
| 个体生成步骤二：个人记忆 | 阶段 4：`PersonalGenesisPlan` |
| 个体生成步骤三：校验与完成 | 阶段 5：`GenesisBundle` 与最终所有者提交 |

七层世界设定、居民可观察知识、A/B 配置资料、六个技术阶段和最终三个所有者不是互相竞争的设计；
它们分别是内容来源、知识视角、机器输入、实现阶段和提交归属。确定性程序负责事实和结构，模型只
负责受限的语言投影；提交完成后创建期资料被销毁，普通 Brain 不回读 Genesis。
...
| v0.3 | 当前唯一活动的完整设计草案；仍需在 Draft 中完成最终确认后再提升为正式设计 |
