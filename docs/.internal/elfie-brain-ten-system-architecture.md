...
> 确认日期：2026-08-12
> 最近修订：2026-08-12
> 性质：Brain 的概念架构、职责边界、运行关系、对抗检查与渐进实现优先级
> 不代表：当前源码已经实现本文能力，也不在本阶段固定目录、Schema、阈值和通信协议

本文建立在以下上位设计之下：

- [Elfie 整体故事、生命形态与功能体系](./elfie-overall-story-and-functional-system.md)；
- [Elfie 顶级模块与目录规约](./elfie-top-level-module-contract.md)。
...
- **失败/恢复**：重启后恢复关键状态、当前身体权威和必要记忆，不把模型不可用误判为 Elfie 消失；
