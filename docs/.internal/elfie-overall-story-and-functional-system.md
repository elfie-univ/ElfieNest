...
> [Elfie 顶级模块与目录规约](./elfie-top-level-module-contract.md)为准，大脑内部实现另由专项设计继续细化
...
Skills 在产品语义上属于大脑皮层的能力，不是和 Brain、Body、Communication 并列的一套生命器官。
模型、Provider、工具沙箱和实际调用设施可以由外部 AI Runtime 提供，但它们不拥有 Elfie 的人格、
记忆和行动权。

Tool Observation 是某个认知回合内部的证据返回，不是第四类外部生命线路。任何 Tool 如果实际要
对人发送内容或让身体行动，最终仍必须形成受限指令，分别经过 Communication 或 Nervous System。
...
活动系统必须把 `VALIDATED`、`NEEDS_CLARIFICATION` 或 `REJECTED` 等创建结果同步反馈给当前大脑
回合，使大脑能当场继续问人，而不是先保存一个残缺活动。已创建活动只保留执行真正需要的上下文
胶囊；执行时还要结合最新记忆、关系和世界状态，不能复制出另一个长期人格。
...
- 睡眠整理不能直接对外发消息或移动身体；需要外部行为时创建活动或内部触发新回合。
...
行为。Brain 是最复杂的子系统，后续仍需分别细化记忆、情绪、能量、皮层、活动和离线认知。
...
提供模型、Provider、工具、Skill 执行、隔离和调用观测。它可以被大脑皮层使用，但不是另一只
...
16. Skills 属于大脑皮层能力，执行设施不能取得人格和行动权；
...
现场见闻、聊天内容、时间、情绪或好奇心可以产生一个活动提案。活动在创建时解析人物关系和可用
联系方式，随后以 Internal Trigger 开启新回合。大脑决定“找谁、为什么、说什么”，Communication
负责接通正确渠道并返回发送结果。主动发起通信不要求先有一条对方消息。
...
离线认知可以发现未兑现承诺或值得继续的问题，但不能趁睡眠直接发消息或移动身体。它形成活动
提案或醒后内部触发，经过新的在线回合、当前状态校验和预算判断后再行动。
...
| Tool 返回网页指令要求发信 | Tool 变成隐藏的第三条执行线路 | Tool 结果只是 Observation；外部行为重新形成受限通信或具身指令 |
...
| 离线整理发现一个有趣想法 | 睡眠系统绕过在线治理发消息 | 只形成 Activity Proposal/Internal Trigger，醒后新回合再决定 |
