...
Supported Provider product metadata has a separate versioned catalog. The bundled
...
future feature. Catalog products provide the official display name, endpoint,
protocol, authentication type, discovery strategy and default model hints used
by the Owner UI. They never contain user credentials. `configs/providers.yaml`
remains user connection-instance configuration and must not be confused with
the metadata catalog.

One catalog product may have multiple configured connections. The storage layer
generates a readable immutable ID such as `anthropic_api_0001`; users edit only
the optional alias and the credential for a known product. Custom connections
add endpoint, protocol and authentication fields, but their ID is still
generated by the backend. Renaming an alias never changes references. Each
connection model keeps its endpoint-specific `id`, display name, discovery
source and optional advanced capability limits. `canonical_model_id` is an
optional system/catalog hint: the UI does not require users to invent it, and
the comparison matrix falls back to a normalized display name when it is absent.
...
also written to
`reports/provider-validations/<connection_id>/latest.yaml` and immutable
`history/` entries. Model benchmarks use the same latest-plus-history pattern
under `reports/model-validations/<connection_id>/`, with an opaque hashed model
directory so an endpoint model ID never becomes a path. The Owner model matrix
groups equivalent display models while preserving the exact endpoint model ID
for every connection cell and benchmark request.
...
Every assigned model is stored as the strict
`<connection_id>/<endpoint_model_id>` reference. Runtime never guesses a
connection from a product name or silently crosses to another subscription.
...
│       └── oauth/
│           └── <connection_id>.json # structured refreshable OAuth credentials
├── reports/
