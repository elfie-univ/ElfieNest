...
| BRN-002 | P0 | open | `BrainContext` now carries Orientation, Selfhood, immutable Profile anchors, versioned Memory state, a versioned Motivation snapshot and a bounded Activity projection with provenance, revisions, explicit unknown/stale states and current-turn cutoffs. Activity remains authoritative in its Brain-owned Store; the existing owner-memory compatibility write remains open. | Context assembly reads all versioned owner snapshots without hidden writes, distinguishes facts/inferences/unknowns, and records all snapshot cutoffs used by one Turn; the remaining Memory owner-write path is removed. |
