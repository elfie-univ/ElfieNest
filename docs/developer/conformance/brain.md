...
| BRN-002 | P0 | open | `BrainContext` now carries Orientation, Selfhood, immutable Profile anchors and a versioned Memory-state projection with provenance, revisions, unknown fields and current-turn cutoffs. Activity is authoritative in its Brain-owned Store and visible in the Lab projection, but is not yet part of the unified BrainContext snapshot; Motivation and the existing owner-memory compatibility write remain open. | Context assembly reads all versioned owner snapshots without hidden writes, distinguishes facts/inferences/unknowns, and records all snapshot cutoffs used by one Turn. |
