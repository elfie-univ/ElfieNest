...
| BRN-002 | P0 | open | `BrainContext` now carries Orientation, Selfhood, immutable Profile anchors and a versioned Memory-state projection with provenance, revisions, unknown fields and current-turn cutoffs; Motivation and Activity still lack authoritative snapshots, and the existing owner-memory compatibility write remains. | Context assembly reads all versioned owner snapshots without hidden writes, distinguishes facts/inferences/unknowns, and records all snapshot cutoffs used by one Turn. |
