...
| --- | --- | --- | --- | --- |
| BRN-001 | P0 | open | `PerceptionFrame` can contain physical, social, execution and internal payloads together and has no authoritative `SourceDomain`, `InteractionScope`, `ResponseScope` or one-domain validator. | Communication, Embodied and Internal lanes produce separate typed `TurnFrame` values; different conversations, body generations and internal causes remain separate scopes; simultaneous cross-domain events remain separate Turns; a Communication Turn asking for motion cannot commit a body directive. |
