...
| ELF-003 | P0 | open | Brain already owns a typed `FoodPort`, but still uses the broad historical `CorticalRuntimePort` for model access and tool execution; the final `ModelPort` and `ToolPort` are absent. | Brain owns separate typed `FoodPort`, `ModelPort` and `ToolPort`; Food preserves named-role/single-fallback/Emergency behavior; the scoped Tool Adapter retains technical safety veto; Bootstrap injects the Infrastructure views; all callers migrate and the broad Runtime bridge is removed. |
