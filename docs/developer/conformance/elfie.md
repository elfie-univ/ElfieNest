...
> records the life-system work adopted by contract version 2.x.
...
| ELF-010 | P0 | open | The earlier broad Profile fields are gone, but `ElfieProfile` still persists `ProfileProvenance` (`generator_version`, seeds and `user_choices`), embodiment capability references, arrival mode/base and a code-owned world/species Canon. The Facade also rebuilds a live Canon-rich `ProfileAnchorSnapshot`; fixed gender and the accepted stable age anchor are not owned by the strict Profile dossier. | Reduce Profile to the contract 2.3 allowlist; move arrival/training/history to Memory and capabilities to Body/Brain; remove generation inputs and Canon/world projections; make authorized pages aggregate owner projections without writing them into Profile. Close only after fresh-create, reopen, projection and recursive reference tests prove no second source. | target=Elfie 2.3 Profile clauses and ADR-0033; inventory=`elfie/profile/{models.py,generator.py,canon.py}`, `elfie/brain/selfhood/contracts.py`, `elfie/facade_operations.py`, adoption/API projections and stores; references=field/reference inventory; verification=pending typed boundary, persistence/reopen and observer tests; residuals=existing workspace policy must be decided under normal data-change governance. |
...
| ELF-013 | P0 | open | The public Genesis contracts and Memory committer exist, but `infrastructure/persistence/elfie_workspace/adoption_profiles.py` still compiles Selfhood, personal knowledge, names, relationships, episodes and the full Genesis bundle while persisting files. Accepted adoption records also carry generation/personality/story inputs beyond the transaction, and the current manifest/seed shape is treated as replay provenance. | Move all life-semantic compilation into `elfie/genesis/`; leave App `resident_admission` with orchestration and Infrastructure with typed load/save only. Treat questionnaire, `LifeContext`, plan, source binding and seeds as in-flight data, delete them after commit/abort, and retain only a minimal non-semantic technical receipt outside Profile. Prove atomic final-owner publication, bounded recovery and runtime source isolation. | target=Elfie 2.3 Genesis clauses, Application 1.11 and ADR-0033; inventory=`elfie/genesis/`, adoption candidate/reservation flow, resident admission, workspace materializer and stores; references=semantic-decision call-chain inventory; verification=pending crash-window, idempotency, input-disposal, restore-without-source and architecture tests; residuals=current source placement and existing data remain unmigrated. |
...
The earlier Ports/Adapters and life-system rows retain their evidence, but
contract 2.3 reopens the exact Profile and Genesis ownership gaps in ELF-010 and
ELF-013. This register is not a second runtime authority or permission to add
...
4. continuous life state closed ELF-017 and established Selfhood/Energy/Orientation owners; strict Profile cleanup remains open in ELF-010;
...
7. Genesis closes ELF-013 only after semantic compilation moves to `elfie/genesis`, creation inputs are destroyed and final-owner/source-isolation evidence exists.
