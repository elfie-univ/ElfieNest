...
> records the life-system work adopted by the current contract version 2.3.
...
| ELF-011 | P0 | closed | Brain now owns private cognitive coordination and context assembly; typed single-domain Turns and host-enforced response scope are in place, and the former root cognitive files are removed. | Focused Brain lifecycle, lane, scope and decision-boundary tests pass; Elfie Lab shows the source domain, Scope, decision and delivery receipt for the communication loop. Keep this boundary ratchet green while later Brain capabilities are added. | target=ELF-011 Brain Turn ownership; inventory=elfie/brain/runtime and Lab; references=root cognition scan; verification=Brain lifecycle/Lab tests; residuals=the newer Activity-domain and embodied-control requirements are tracked by ELF-018 |
...
| ELF-017 | P0 | closed | Orientation and Selfhood are independent authorities. Energy, Memory, Orientation, Selfhood, Motivation and Cognitive Consolidation participate in one continuity checkpoint; short-lived Emotion is explicitly process-local and returns to its personality-derived baseline on sleep or restart. Orientation candidates derive from current Body generation, conversation, location and Activity and commit only through Turn Settlement. | Focused state, settlement and cross-module recovery tests cover explicit ownership, source/version rules, durable-owner restore, process-local Emotion restart, stale checkpoint rejection and single-message resistance for personality and norms. | target=ELF-017 continuous life state; inventory=Brain state owners and continuity; references=checkpoint/settlement guards and ADR-0030; verification=state and cross-module recovery tests; residuals=none |
| ELF-018 | P0 | open | The latest embodied contract is not yet fully implemented: source code still exposes the legacy `Internal` Brain domain and fixed decision variants, while the dynamic capability call, one-causal-window embodied terminal outcome, complete Body/NervousSystem return routing and Godot vertical slice remain incomplete. | Migrate the implementation to exactly `Communication`/`Embodied`/`Activity`; register and validate dynamic capabilities; keep `ACCEPTED`/`STARTED` in the ledger; publish one terminal embodied outcome plus compatible body facts through EventWorkspace; prove Body/NervousSystem round-trip and Godot movement/hearing/vision/touch/position evidence. | target=ADR-0033 and Brain/Elfie/System/Nest-Godot contracts v1.5/2.3/1.10/1.2; inventory=Brain workspace/decision types, Body/NervousSystem, Godot Adapter/Transport/Gateway and vertical-slice plan; references=legacy `Internal` source naming and fixed `DecisionIntent`; verification=focused Brain/Body/Godot tests plus one replayable end-to-end scenario; residuals=implementation unchanged in this documentation update |
...
behavior evidence and permanent machine ratchets. Contract 2.3 reuses those
boundaries and existing baselines; it deliberately creates no second legacy
baseline. The register is evidence for future closure; it is not a second runtime
