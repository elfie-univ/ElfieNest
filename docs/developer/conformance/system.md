...
| --- | --- | --- | --- | --- |
| SYS-001 | P0 | in progress | Root `infrastructure/` now has the seven target capability areas and owns Data Home plus the raw Godot Gateway, but several Adapters still delegate to `ai_runtime/`, `godot_runtime/` or core-owned technical implementations. | Technical implementations move to owned `models`, `tools`, `persistence`, `godot`, `devices`, `communication` or `platform` subareas; old technical roots and compatibility imports are deleted while root `godot_project/` remains unchanged. |
