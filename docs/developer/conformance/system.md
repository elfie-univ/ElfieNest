...
| --- | --- | --- | --- | --- |
| SYS-001 | P0 | in progress | Root `infrastructure/` now owns the seven target capability areas, their local governance, Data Home, persistence, model/provider technology, tools, Godot Gateway, authority hosting and artifact validation. The former `app/infrastructure/` and `godot_runtime/` roots are gone. Remaining ownership debt is concentrated in the registered mixed `ai_runtime/` and Observer/Core residuals. | Keep moving only behavior-equivalent technical leaves into the owned capability areas; retain explicitly mixed coordinators for later Elfie/Nest work; never restore old roots or compatibility imports. |
...
| SYS-004 | P0 | in progress | Service and interactive entry scripts now request Runtime, storage, Nest Session and Elfie restoration from `app/bootstrap/`, and Runtime/Godot channels start through Lifecycle. The API lifespan still constructs and starts its authenticated management WebSocket gateway, so production composition and lifecycle ownership are not yet fully singular. | Bootstrap constructs that remaining concrete gateway and injects startup/shutdown callbacks that delegate channel control to Lifecycle; Interfaces retain only protocol mapping. |
