...
| --- | --- | --- | --- | --- |
| SYS-001 | P0 | in progress | Root `infrastructure/` now has the seven target capability areas and owns Data Home, Godot Gateway, authority hosting and artifact validation. Remaining ownership debt is concentrated in `ai_runtime/`, the registered Observer semantic residual and core-owned technical implementations. | Technical implementations move to owned `models`, `tools`, `persistence`, `godot`, `devices`, `communication` or `platform` subareas; old technical roots and compatibility imports are deleted while root `godot_project/` remains unchanged. |
