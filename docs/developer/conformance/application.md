...
Routes and production callers: /api/v1/admin/model-providers catalog, connection, model, validation, matrix, benchmark and Ollama resources; management Provider client; legacy CLI configuration entry
Target public facade and models: Providers administration facade; strict commands, queries and results for the existing CRUD, lifecycle, verification, refresh, matrix, benchmark and local Ollama behaviors
Ports and adapters: consumer-owned narrow Ports express catalog/connections, local Ollama state and technology, Food reference protection and technical probe/discovery/report work; root Infrastructure Models and Persistence adapters implement them and Bootstrap injects them
...
Legacy deletion list: CLI runtime.yaml/auth.env v1 write chain
