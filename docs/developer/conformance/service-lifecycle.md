...
| WP2 — source shell and checkout control state | `elfienest.sh` is a bootstrap; the persistent Python shell owns in-memory context, shared parsing/help, TTY selection and scoped target environment. Protected history/candidates live under ignored `.elfienest-cli.local/` and are revalidated. | `elfienest.sh`, `scripts/elfienest.py`, `app/interfaces/cli/target_context.py`, `infrastructure/platform/source_cli_state.py`, `.gitignore`. | Focused shell, parser, source-state and no-fallback tests pass; entering the shell does not create product data. |
