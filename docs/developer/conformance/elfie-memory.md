...
| --- | --- | --- | --- | --- | --- | --- |
| MEM-001 | P0 | closed | The old entity/subtype and embedded-edge layout has been replaced for the target Adapter by the reviewed Episode, node, assertion and evidence tables. | Target tables, constraints, ownership and rebuildable lexical projections are implemented without a second edge authority. | target=design §9.1–9.2; inventory=`infrastructure/persistence/memory/schema.py`, `node_store.py`, `edge_store.py`; references=persistence scan; verification=target schema/round-trip/reopen tests and 315 affected tests; residuals=an existing live legacy file still requires the explicit importer/cutover below. | None for the development target. |
