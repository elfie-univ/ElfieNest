...
> Status: implementation complete on the current branch; the real Ark machine gate passed, but the exact-candidate soft gate has one `memory_grounding=3` sample, so Stage 1 promotion remains blocked pending remediation, owner review and production cutover<br>
...
| MEM-008 | P0 | blocked | Deterministic structural gates and the authorized real Ark run pass its machine gate, but one exact-candidate soft-quality sample is below threshold; owner experience review is not complete, so Stage 1 is not promoted. | A replayable redacted report must show deterministic gates, source grounding, relationship/conflict behavior, restart and latency before Stage 1 promotion. | target=design §9.7 and `docs/.internal/elfie-stage1-memory-backed-chat-execution-plan.md`; inventory=`devtools/evals/stage1_chat_ark.py`, scenario set and focused tests; references=`build/evaluations/stage1-chat/e1-ark-real-136eebaa/report.json`; verification=86 deterministic E1 tests passed, 33/33 repeated machine scenarios passed, real Ark candidate run completed with one provider empty response recovered through the existing failure path and judge 33 calls; machine gate passed, but `world-species-common` repetition 2 scored `memory_grounding=3` (worst; other dimensions met the threshold), persistence scan exit 0; targeted 3-repetition diagnostic for that scenario passed its machine and soft gates; residuals=remediate/re-run the complete soft gate, owner experience review and separate production-data cutover remain. | Ark authentication and structured judge completion passed; no secret was written to the report. |
