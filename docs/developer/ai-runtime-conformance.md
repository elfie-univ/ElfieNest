...
| AR-018 | P1 | open | There is no end-to-end contract test for Provider to model evidence to food to Elfie execution to emergency fallback. | A clean temporary `ELFIE_HOME` passes all 13 steps in the contract, including discovery precedence, comparison snapshots, lifecycle guards and `no_available_food`. |
