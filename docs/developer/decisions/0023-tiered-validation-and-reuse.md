...
- Keep exact-candidate reuse, and give the remaining expensive G3 backstop a
  separate fingerprint. Root `task-closure*.json` matrices are omitted only
  from that backstop fingerprint because their scope, secret and
  complete-closure checks are rerun for every candidate. Source, tests,
  dependencies, toolchains, documentation and gate rules remain fail-closed
  backstop inputs.
- After a failure, rerun the exact failed node first and expand to its owning
  ...
  requires it. Run a complete G3 backstop once for the final executable
  candidate, not after every repair edit.
- The keys cover rule/check version, base SHA, selected candidate content,
  ...
  main backstop reuses valid bundles; `--no-cache` is propagated to force a
  clean replay.
...
remain fail-closed. A closure-only evidence edit no longer reruns unchanged test
bundles, dependency installs and the docs build after that executable backstop
already passed; its candidate-specific checks still run. Latest-SHA CI remains
mandatory for protected delivery.
