...
## 0.1.0-beta.4 internal-test desktop installer

We currently build only internal-test installers: the version is pinned to
`0.1.0-beta.4`, with no auto-update configured and no model weights, Ollama engine or
...
`v0.1.0-beta.4`, runs the same matrix, validates the native installer contents,
...
```bash
git tag -a v0.1.0-beta.4 -m "ElfieNest 0.1.0-beta.4"
