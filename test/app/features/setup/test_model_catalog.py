...
def test_setup_offline_draft_accepts_only_bare_catalog_ids() -> None:
    for model_id in ("qwen2.5:0.5b", "qwen3.5:0.8b", "gemma3:270m"):
        request = SetupOfflineDraftRequest(use_local_ollama=True, model_id=model_id)
        assert request.model_id == model_id

    for model_reference in ("ollama/qwen2.5:0.5b", "qwen2.5:7b", "openai/gpt-5"):
        with pytest.raises(ValidationError):
            SetupOfflineDraftRequest(use_local_ollama=True, model_id=model_reference)
