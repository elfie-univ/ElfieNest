...
def _supervisor(
    ...
    **kwargs,
    ...
    if core_process_identity is None:

        def default_core_process_identity(pid: int) -> ProcessSnapshot:
            return ProcessSnapshot(
                pid=pid,
                cwd=Path("/repo"),
                command=("/repo/.venv/bin/python3", "/repo/scripts/serve.py"),
                birth_identity=f"birth-{pid}",
            )

        core_process_identity = default_core_process_identity
