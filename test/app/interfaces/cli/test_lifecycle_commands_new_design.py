...
class _LaunchSupervisor:
    ...
    def recover_owned(self, **kwargs) -> ServiceLifecycleResult:
        self.events.append(f"recover:{kwargs['owner_id']}:{kwargs['expected_generation']}")
        return self.start_result
...
def test_controller_wait_reads_one_runtime_in_one_process(capsys, tmp_path: Path) -> None:
    projections = [
        RuntimeSnapshotV1(
            instance_id="test-instance",
            generation=1,
