...
def _client(tmp_path: Path):
    ...
    patches = (
        ...
        patch("elfienest.api.tool_admin_routes._RUNTIME_CONFIG_PATH", config_path),
        patch("elfienest.api.tool_admin_routes.set_tool_secret"),
    )
...
def test_tool_config_round_trip_uses_runtime_policy_and_local_secret(tmp_path):
    ...
    with patches[0], patches[1], patches[2], patches[3] as secret_writer:
        ...
        with TestClient(app) as client:
            ...
            initial = client.get("/api/admin/runtime/tools/", headers=headers)
            saved = client.put(
                "/api/admin/runtime/tools/web_search",
                json={
                    "enabled": True,
                    "provider": "brave",
