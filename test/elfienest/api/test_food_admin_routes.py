...
def client(tmp_path, paths):
    ...
    with (
        ...
        patch("elfienest.api.food_admin_routes._FOOD_CATALOG_PATH", paths["catalog"]),
        patch("elfienest.api.food_admin_routes._FOOD_HISTORY_DIR", paths["history"]),
        patch(
            "elfienest.api.food_admin_routes._MODEL_EVIDENCE_PATH", paths["evidence"]
        ),
    ):
...
def test_food_update_is_previewed_and_requires_confirmation(client, paths):
    ...
    status = client.get("/api/admin/runtime/foods/update-status", headers=headers)
    preview = client.post(
        "/api/admin/runtime/foods/update-preview",
        json={"use_llm": False},
        headers=headers,
    )
    rejected = client.post(
        "/api/admin/runtime/foods/update-apply",
        json={"confirm": False},
        headers=headers,
    )
...
def test_food_can_be_edited_without_separate_expert_mode(client):
    headers = auth(client)
    response = client.put(
        "/api/admin/runtime/foods/standard",
        json={
            "display_name": "标准粮",
            "description": "人工调整",
