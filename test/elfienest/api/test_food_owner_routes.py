...
def client(tmp_path, paths):
    ...
    with (
        ...
        patch("elfienest.api.food_owner_routes._FOOD_CATALOG_PATH", paths["catalog"]),
        patch("elfienest.api.food_owner_routes._FOOD_HISTORY_DIR", paths["history"]),
        patch(
            "elfienest.api.food_owner_routes._MODEL_EVIDENCE_PATH", paths["evidence"]
        ),
    ):
