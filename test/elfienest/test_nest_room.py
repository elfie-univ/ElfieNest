...
class TestElfieNestSimulation(unittest.TestCase):
    ...
    def test_room_logic_and_broadcast(self):
        ...
        self.assertEqual(room.room_state["elfies_status"]["艾菲"]["target_furniture"], "bed_1")
        self.assertEqual(room.room_state["furniture"]["bed_1"]["occupant"], "艾菲")
        
        # 精灵艾菲搬去 chair_1
        room.update_elfie_posture("艾菲", "sitting", "chair_1")
        self.assertEqual(room.room_state["furniture"]["bed_1"]["occupant"], None) # 原床位自动释放
        self.assertEqual(room.room_state["furniture"]["chair_1"]["occupant"], "艾菲") # 新椅子被占用
    ...
    def test_engine_coordinator_reflex_and_main_compatibility(self):
        ...
        # 模拟 Godot 握手上报
        engine.api_server._trigger_callbacks("register_scene", {
            "furniture": ["bed_1", "chair_1"],
            "cameras": ["overview"]
        })
        self.assertIn("bed_1", engine.room.room_state["furniture"])
        
        # 模拟 Godot 模型寻路到达反馈
        engine.api_server._trigger_callbacks("arrived_at", {
            "elfie_id": "艾菲",
            "target": "bed_1"
        })
        self.assertEqual(engine.room.room_state["elfies_status"]["艾菲"]["posture"], "lying")
        self.assertEqual(engine.room.room_state["furniture"]["bed_1"]["occupant"], "艾菲")
