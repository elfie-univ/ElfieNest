    ...
    def test_skills_evolution_write(self):
        ...
        with patch.object(agent.registry, 'get_model_info') as mock_info:
            ...
            with patch.object(agent, '_call_llm_api') as mock_api:
                ...
                with patch.object(agent.skills_evolution_plugin, 'write_skill') as mock_write:
    ...
    def test_skills_evolution_run(self):
        ...
        with patch.object(agent.registry, 'get_model_info') as mock_info:
            ...
            with patch.object(agent, '_call_llm_api') as mock_api:
                ...
                with patch.object(agent.skills_evolution_plugin, 'run_skill') as mock_run:
                    mock_run.return_value = {"exit_code": 0, "stdout": "3", "stderr": ""}
    ...
    def test_list_skills_interception(self):
        ...
        with patch.object(agent.registry, 'get_model_info') as mock_info:
            ...
            with patch.object(agent, '_call_llm_api') as mock_api:
                ...
                with patch.object(agent.skills_evolution_plugin, 'list_skills') as mock_list:
...
class TestMultimodalPayload:
    ...
    def test_assemble_multimodal_ollama(self):
        ...
        try:
            result = agent._assemble_multimodal_payload(messages, [temp_img], None, "ollama")
    ...
    def test_assemble_multimodal_cloud(self):
        ...
        try:
            result = agent._assemble_multimodal_payload(messages, [temp_img], None, "openai")
            # 云端格式为 list
    ...
    def test_assemble_multimodal_missing_file(self):
        ...
        with pytest.raises(FileNotFoundError):
            agent._assemble_multimodal_payload(messages, ["nonexistent.jpg"], None, "ollama")
    ...
    def test_assemble_audio_payload(self):
        ...
        try:
            result = agent._assemble_multimodal_payload(messages, None, temp_audio, "openai")
            content = result[-1]["content"]
            assert isinstance(content, list)
            # 验证有音频内容
            has_audio = any(
                isinstance(c, dict) and c.get("type") == "input_audio"
                for c in content
            )
            assert has_audio
        finally:
...
class TestCallLLMApi:
    ...
    def test_ollama_api_call(self):
        ...
        with patch('urllib.request.urlopen') as mock_urlopen:
            mock_response = Mock()
            mock_response.read.return_value = b'{"message": {"content": "test response"}}'
    ...
    def test_cloud_api_call(self):
        ...
        with patch('urllib.request.urlopen') as mock_urlopen:
            mock_response = Mock()
            mock_response.read.return_value = b'{"choices": [{"message": {"content": "cloud response"}}]}'
            mock_urlopen.return_value.__enter__.return_value = mock_response
