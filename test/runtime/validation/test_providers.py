...
from runtime.validation.models import CheckStatus
...
def test_discovers_ollama_models(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "runtime.validation.providers.urllib.request.urlopen",
...
def test_discovers_openai_compatible_models_with_bearer_header(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "runtime.validation.providers.urllib.request.urlopen", fake_urlopen
    )
...
def test_model_discovery_failure_becomes_check_result(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("runtime.validation.providers.urllib.request.urlopen", fail)
...
def test_discovery_uses_manual_models_when_models_endpoint_is_unavailable(
    ...
    monkeypatch.setattr(
        "runtime.validation.providers.urllib.request.urlopen", unsupported
    )
