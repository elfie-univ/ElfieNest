...
class TestEmbodiedPerception(unittest.TestCase):
    ...
    def test_morphological_restrictions(self):
        ...
        response_quad = quad_elfie.perceive_and_respond(sensor_data_quad, MockQuadAgent())
    ...
    def test_cerebellar_gait_cpg(self):
        ...
        # 校验方向相反
        self.assertTrue((left_hip > 0 and right_hip < 0) or (left_hip < 0 and right_hip > 0))
        
        # 模拟 0.5s 时的摆臂： 左臂与左脚反相位，左肩 (left_shoulder) 与右大腿同相位
        left_shoulder = joints_t05["left_shoulder"]
        self.assertEqual(math.copysign(1.0, left_shoulder), math.copysign(1.0, right_hip))
    ...
    def test_vision_viewport_perception(self):
        ...
        analysis = elfie.speech_actuator.synthesize_speech("眼前的红木门紧闭着哒", elfie.anatomy.voice_profile)
