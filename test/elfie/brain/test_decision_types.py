...
def test_internal_and_noop_intent_discriminators_round_trip() -> None:
    # Given: the two non-external variants in the closed intent union.
    intents: tuple[DecisionIntent, ...] = (
        InternalIntent(
            type="internal",
            intent_id=IntentId("internal-1"),
            cause_event_ids=(EventId("social-event"),),
            dependency_ids=(),
            deadline=PLAN_DEADLINE,
            cancel_policy=CancelPolicy.IF_NOT_STARTED,
            operation=InternalOperation.REMEMBER,
    ...
    # When: each variant crosses its discriminated JSON boundary.
    adapter = TypeAdapter(DecisionIntent)
    restored = tuple(
        adapter.validate_json(adapter.dump_json(intent)) for intent in intents
    )

    # Then: each discriminator selects exactly its declared model.
    assert restored == intents
    assert tuple(intent.type for intent in restored) == ("internal", "noop")
