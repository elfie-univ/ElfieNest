...
from elfie.brain.journal import BrainJournal, BrainJournalKind, InMemoryBrainJournal
...
def test_router_journals_acceptance_and_terminal_receipt() -> None:
    ...
    batch = router.submit(_communication_decision(plan))
    assert isinstance(batch, ExecutionBatch)
    router.wait_for_turn(plan.turn_id, timeout=1)

    entries = journal.entries()
    assert any(entry.kind is BrainJournalKind.DIRECTIVE_ACCEPTED for entry in entries)
    assert [
        entry.status
        for entry in entries
        if entry.kind is BrainJournalKind.EXECUTION_RECEIPT
    ] == ["accepted", "started", "completed"]
    router.stop()
    router.join()


def test_router_journals_rejection_before_execution() -> None:
    store = InMemoryBrainJournal()
    journal = BrainJournal(elfie_id=ELFIE_ID, store=store, clock=lambda: NOW)
    router = OutputRouter(
        elfie_id=ELFIE_ID,
    ...
    rejected = router.submit(_communication_decision(_plan((_message(0),))))

    assert not isinstance(rejected, ExecutionBatch)
