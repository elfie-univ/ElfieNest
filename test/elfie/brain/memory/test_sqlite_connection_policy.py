...
from elfie.brain.memory.sqlite_connection import (
    SQLitePathError,
    memory_sqlite_connection,
)
...
def test_connection_enables_rows_foreign_keys_and_private_mode(tmp_path: Path) -> None:
    ...
    with memory_sqlite_connection(db_path) as connection:
...
def test_connection_rejects_symlink_database(tmp_path: Path) -> None:
    ...
    with pytest.raises(SQLitePathError):
        with memory_sqlite_connection(link):
            pass
...
def test_connection_rejects_symlink_parent_before_writing(tmp_path: Path) -> None:
    ...
    with pytest.raises(SQLitePathError):
        with memory_sqlite_connection(linked_parent / "knowledge.sqlite"):
            pass
...
def test_connection_rejects_non_directory_ancestor_before_writing(
    ...
    with pytest.raises(SQLitePathError):
        with memory_sqlite_connection(file_ancestor / "knowledge.sqlite"):
            pass
