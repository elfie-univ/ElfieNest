...
def test_bundle_inputs_are_conservative_but_do_not_include_unrelated_roots() -> None:
    ...
    assert "task-closure.json" not in inputs
...
def test_focused_test_fingerprint_ignores_closure_but_not_source(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(validation_cache, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(test_bundles, "PROJECT_ROOT", tmp_path)
    ...
    paths = ["app/a.py", "task-closure.json"]

    first = test_bundles.focused_test_fingerprint("base", ["test/app"], paths)
    closure.write_text('{"status": "complete"}\n', encoding="utf-8")
    after_closure = test_bundles.focused_test_fingerprint("base", ["test/app"], paths)
