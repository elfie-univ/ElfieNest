...
def test_command_selection_keeps_g1_focused_and_adds_g2_quality() -> None:
    ...
    affected_command = next(
        command
        for label, command in _commands(commit_plan, "base", "task-closure.json")
...
def test_no_cache_is_forwarded_to_the_controlled_affected_test_runner() -> None:
    ...
    affected_command = next(
        ...
        for label, command in _commands(
            plan, "base", "task-closure.json", no_cache=True
        )
        if label == "affected tests"
    )
...
def test_backstop_fingerprint_ignores_only_task_closure_metadata(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(validation_cache, "PROJECT_ROOT", tmp_path)
    closure = tmp_path / "task-closure.json"
    source = tmp_path / "changed.py"
    guide = tmp_path / "guide.md"
    closure.write_text('{"status": "verifying"}\n', encoding="utf-8")
    source.write_text("value = 1\n", encoding="utf-8")
    guide.write_text("first\n", encoding="utf-8")
    paths = ["task-closure.json", "changed.py", "guide.md"]

    first = backstop_fingerprint("base", paths)
    closure.write_text('{"status": "complete"}\n', encoding="utf-8")
    after_closure = backstop_fingerprint("base", paths)
    ...
    assert first == after_closure
    assert after_source != first
    assert after_documentation != first


def test_main_reuses_backstop_but_rechecks_current_metadata(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(
        validation_gate, "changed_paths", lambda _base: ["task-closure.json"]
    )
    monkeypatch.setattr(
        validation_gate,
    ...
    result = validation_gate.run_stage(
        "main", "base", "task-closure.json", tmp_path, False, False
    )
...
def test_main_no_cache_reaches_direct_backstop_without_reusing_bundles(
    ...
    result = validation_gate.run_stage(
        "main", "base", "task-closure.json", tmp_path, True, False
    )
