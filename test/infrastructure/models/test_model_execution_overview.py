...
def test_model_execution_defaults_do_not_fallback_to_python_literals(
    monkeypatch, tmp_path
):
    source_path = (
        Path(__file__).resolve().parents[3] / "config" / "app" / "system-defaults.yaml"
    )
    document = yaml.safe_load(source_path.read_text(encoding="utf-8"))
    del document["system"]["model_execution"]["temperature"]
    root = tmp_path / "bundled"
    path = root / "app" / "system-defaults.yaml"
...
def test_overview_store_keeps_current_and_history(tmp_path):
    store = ModelExecutionOverviewStore(tmp_path)
...
def test_overview_store_defaults_to_model_validation_directory(monkeypatch, tmp_path):
    ...
    # When
    store = ModelExecutionOverviewStore()
