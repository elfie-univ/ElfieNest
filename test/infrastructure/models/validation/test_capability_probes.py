...
def test_capability_evaluation_requires_channel_specific_evidence() -> None:
    tools = evaluate_capability_probe(
        "tools", "", {"tool_call_count": 1}
    )
    vision = evaluate_capability_probe(
        "vision", "ELFIENEST_VISION_OK", {}
    )
...
def test_provider_rejection_is_verified_unsupported_but_network_failure_unknown() -> None:
