"""tests for ai_runtime.storage.migration module"""
...
from ai_runtime.storage.migration import (
...
def test_migrate_no_old_data(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "ai_runtime.storage.migration._OLD_DATA_DIR", tmp_path / "nonexistent_data"
    )
    monkeypatch.setattr(
        "ai_runtime.storage.migration._OLD_RUNTIME_CONFIG",
...
def test_runtime_config_json_conversion(tmp_path):
    ...
    import ai_runtime.storage.migration as migration_mod
