...
S 级本地 commit 只运行与改动直接相关的最小检查；CSS-only 等局部改动不得因为 commit
自动启动服务、Godot、浏览器、远端同步或全套门禁。M/L 级本地 checkpoint 使用
`scripts/pre_submit_gate.sh --stage commit`；功能分支推送使用 `--stage push`。普通提交不在
本地运行完整 G3，也不因 main 前进而反复拉取、合并和重跑；精确 PR head 由可信基础分类器
选择并行 Lane，`elfienest/ci-gate` 通过后由必需的 `elfienest/merge-gate` 包装结果；进入
merge queue 后，同名门禁再以秒级检查合成提交。完整 G3 只用于 main 合并后、显式
`--stage full` 和发布验收。治理、工具链、锁文件和未知影响在 PR 中 fail-closed 选择全部 Lane。
确定性测试证据按检查身份、命令、声明输入内容与模式、本机工具链和必要基础提交计算；同一输入
下 G1 结果可被 G2 复用，完整稳定测试包及覆盖率片段可被 full 后盾复用。精确 node 或单文件
不能冒充更大的测试包；原始 `pytest` 只用于诊断。失败或阻塞结果永不缓存，本地缓存也不能替代
绑定最新候选 SHA 的 CI。
...
- 用户在当前任务中明确说“按计划执行”“提交”“推送”或“合并到远程主分支”时，
  该授权覆盖本任务内必要的拆分、聚焦验证、提交、正常合并和远程状态核验；不得把
  内部拆分、测试分类、门禁分级或技能选择再次当作用户决策询问。
