...
完整后盾在 main 合并后及显式 full/发布验收中把全部 Lane 并行展开；治理、工具链、锁文件和
未知影响在 PR 中 fail-closed 选择全部 Lane。确定性测试证据按检查身份、命令、声明输入内容
...
push、创建或复用 PR、进入 merge queue、核验远端结果，或遇到 GitHub 认证、网络及
worktree 权限错误时，必须读取 `.agents/skills/git-submit-and-push/SKILL.md`。该技能只处理
远端交付和故障恢复，不复制或替代机器质量分类。只有变更完整、所需验证通过并满足用户
授权与人工审阅门时，才能暂存、提交和推送；明确要求“不提交”“审阅后再提交”或
“本地保留”时，保持改动在本地。

### 用户授权的任务级持续有效

- “本地提交”或 `commit` 只授权创建本地 commit；“推送”授权更新当前功能分支；
  “提交代码到主线”“交付”“合并到远程主分支”，或已批准计划中明确包含的远端交付，
  才授权功能分支、PR、merge queue 和远端结果核验。孤立的“提交”按当前已明确上下文解释，
  不从本地语境擅自扩大为远端写入，也不在远端交付语境退缩为只做本地 commit。
- 远端交付一经明确授权，即覆盖本任务内必要的拆分、聚焦验证、提交、正常重试、PR、
  queue 和远程状态核验；不得把内部拆分、测试分类、门禁选择或技能选择再次当作用户决策询问。
- 只要目标、范围和验收条件没有变化，不重复调用用户确认。门禁失败先按“代码失败、
  环境阻塞、远程拒绝、危险操作”分类并继续处理；只有密钥/外部账号、真实数据删除、
  强制改写历史或没有安全替代方案时才暂停请求方向。
