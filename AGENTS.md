...
测试和文档门禁并要求 closure `complete`。确定性测试检查的成功证据按检查身份、命令、声明的
输入内容与模式、本机工具链和必要的基础提交计算，不按 G1/G2/G3 重复记账；同一输入下 G1
跑过的聚焦测试可被 G2 复用，完整稳定测试包及其覆盖率片段也可被 G3 复用。精确 node 或
单文件通过不能冒充更大的测试包；直接运行原始 `pytest` 只用于诊断，不产生门禁复用证据。
输入、命令或工具变化即失效，失败或阻塞结果永不缓存；缓存也不能替代最新 commit SHA 的 CI。
