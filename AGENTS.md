...
本地反馈使用 `scripts/pre_submit_gate.sh --stage commit`，功能分支推送使用
`--stage push`，主线合并/发布使用 `--stage main`；治理、工具链、锁文件和未知影响会
自动升级到主线级别。G1/G2 运行受影响检查，只有 G3 才运行完整不可变架构、质量、测试
和文档门禁。确定性测试检查的成功证据按检查身份、命令、声明的输入内容与模式、本机工具链
和必要的基础提交计算，不按 G1/G2/G3 重复记账；同一输入下 G1 跑过的聚焦测试可被 G2 复用，
完整稳定测试包及其覆盖率片段也可被 G3 复用。精确 node 或单文件通过不能冒充更大的测试包；
直接运行原始 `pytest` 只用于诊断，不产生门禁复用证据。输入、命令或工具变化即失效，失败
或阻塞结果永不缓存；缓存也不能替代最新 commit SHA 的 CI。
