...
在产品版本升级到 `v0.5.0` 之前，ElfieNest 仍按快速 MVP 阶段执行，当前目标是
尽快做出可用、可看、可验证的本地单机产品，而不是维护稳定版升级体验。

- 默认不要为旧数据库、旧目录结构、旧缓存、旧本地配置或历史开发数据设计复杂
  向后兼容迁移；除非用户明确要求保留旧数据，否则优先选择更简单的重建、重置、
  清空测试数据或提供一次性导出/导入路径。
- 遇到需要“迁移旧数据才能继续”的任务时，先向用户说明可选路径，并优先推荐
  备份后重建数据目录，而不是自动展开大规模 schema/workspace 迁移工程。
