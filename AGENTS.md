...
普通本地提交的累计验证预算默认不超过 10 分钟。若必要的已选检查即将超出预算，保留
现有证据并报告具体缺口，不擅自扩大为完整门禁；发布和用户显式要求的完整验证不受该
普通提交预算约束。
...
局部本地 commit 只运行与改动直接相关的最小检查；CSS-only 等局部改动不得因为 commit
自动启动服务、Godot、浏览器、远端同步或全套门禁。涉及 schema、公开 API、持久化、
架构边界或生命周期所有权的本地 checkpoint 使用 `scripts/pre_submit_gate.sh --stage commit`；
功能分支推送使用 `--stage push`。普通提交不在
